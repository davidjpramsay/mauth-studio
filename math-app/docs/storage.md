...
AI/chatbox file operations should go through `apps/web/src/lib/mauthAssistantFileTools.ts` and normally be dispatched by `apps/web/src/lib/mauthAssistantAdapter.ts`. That layer wraps normal project-file operations with explicit tools for listing, opening, saving, save-as, folder creation, duplication, rename, move, delete, version listing, and version restore. It deliberately uses an injected project-file driver so tests can verify behaviour without a browser, while the visible Assistant panel passes the frontend API functions through the adapter. The adapter passes the active file path and loaded revision to file tools; saving the active file must use that loaded revision, not a freshly listed revision, so assistant saves cannot bypass external-change protection. The file tools do not own active editor state or autosave; the adapter host updates the editor and active-file state after opening or saving the active document. Autosave records must include `frontMatter`, `questions`, `formattingConfig`, optional `logo`, and the active project file revision so recovered drafts preserve exam/school-test template choice and page formatting.
...
If the default storage root is used, moving the whole `math-app` folder also moves project files, legacy saved tests, autosave drafts, and logo assets because they live inside `math-app/storage`.
