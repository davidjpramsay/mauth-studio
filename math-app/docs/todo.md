...
- Keep the semantic `graph2d` geometry layer exposed through assistant tools, preview summaries, and evals for source-faithful 2D exam diagrams: named points, segments, angles, equal-length ticks, equal-angle arcs, right-angle squares, and dashed/helper styling should be one compact renderer payload rather than many tiny freehand feature modules.
...
- `mauth.question.upsert` is implemented as the preferred high-level authoring tool. It lets the provider replace one existing question, or append the next missing question, from a compact payload containing question number/id, marks, question text, optional structured parts, optional diagram configs, student-space lines, and solution text. It creates student-only answer space and solution-only solution text automatically, validates the generated blocks/parts, applies the update, and commits through the adapter/history path. It intentionally only appends the immediate next question number, so the assistant can make Question 2 after Question 1 without creating arbitrary numbering gaps. `mauth.author.replaceQuestion` remains as a legacy alias.
- For attachment-derived one-question conversions, `mauth.question.upsert` is also the expected path when the source has a visible diagram and visible parts. The assistant should put a native diagram block under the stem, then structured parts with non-empty part prompts; it should not substitute a prose diagram description or blank part rows.
- The backend now makes this diagram requirement structural for focused attachment conversions that explicitly ask for a diagram/graph/chart or for the source diagram to go underneath the stem: the exposed direct `mauth_question_upsert` tool schema requires `diagram`, so the model should repair instead of silently writing a text-only question.
- Fresh attachment/source-question requests now start as clean provider turns, and add-diagram validation for missing target questions explicitly repairs toward `mauth.question.upsert`/`mauth_convert_source_question`. This prevents stale chat context or a wrong diagram-only tool choice from turning "add this attached question" into a failed edit against a non-existent question.
- `mauth.question.upsert` now renders each structured part prompt as the first text module inside that part before the answer space and solution. The `part.text` field is still stored for summaries, but visible output comes from part content blocks.
- `mauth.author.addDiagram` is implemented for focused diagram follow-ups. It can add or replace one top-level diagram in an existing question from a renderer-specific `graphConfig`; repair calls can pass `diagramId` to replace the bad diagram rather than appending another one. The assistant should choose the diagram renderer intelligently rather than relying on canned `standardDiagram` names.
...
- High-level assistant diagram payloads now also run conservative intent validation before apply. Obvious classroom patterns are routed to the intended native renderer: circle/tangent/chord theorem geometry to `geometricConstruction`, scalar-product source ray diagrams and coordinate/component vectors to `vector2d`, statistics charts to `statsChart`, Venn/set diagrams to `setDiagram`, coordinate/function graphs to `graph2d`, and networks to `network`. Wrong choices fail with repairable `arguments.diagram.graphConfig.type` issues and are covered by the no-cost self-smoke suite.
- The old Penrose relationship-link type has been removed from the public schema and assistant brains. Use `vector2d` for all coordinate vectors, component vectors, and source-faithful scalar-product ray diagrams. Use `network` only for schematic node/link network diagrams where conceptual layout matters more than exact vector geometry.
...
- `mauth.solutions.writeAll` is implemented for whole-test marking-key passes. It requires solution payload coverage for every marked question, part, and subpart, preserves existing shared diagrams, creates/resizes matched student answer spaces, validates hidden mark ticks, and returns a document-wide layout check result.
- `mauth.layout.check` is implemented as a document-wide student/solutions/both-mode inspector. It flags missing student spaces, missing solutions, solution/tick mismatches, rendered overflow risks, oversized diagrams, and print-risk items. With `autoRepair: true`, it repairs safe deterministic issues once before reporting.
- Fresh selected/current preview-inspection, broad layout-check, document-inspection, validation, help, and clarification prompts now use backend native fast paths without spending provider tokens. These deterministic routes can run even when the OpenAI provider key is not configured, and successful local tool outputs now end visibly in the panel instead of making a provider summary call. Broad layout checks use `autoRepair: true` to fix safe mechanical issues once, such as missing student answer spaces, oversized editable diagrams, final blank-page breaks, and unambiguous rendered page overflow from one targeted student space block, then refresh the check after repaint when possible. Remaining content, solution, semantic diagram, and ambiguous layout repairs still go to the provider with concrete warnings.
- `mauth.author.adjustResponseSpaces` is implemented for focused answer-space/working-space edits. It resizes or adds student-only response spaces at question, part, or subpart level while preserving existing shared question text, diagrams, and solution modules.
- `mauth.format.apply` is implemented for focused formatting/layout edits. It can set/remove page breaks before questions, parts, and subparts, align diagrams, adjust answer-space lines, move existing modules, fit solutions to student space, and tidy conservative spacing without rewriting question content.
- `mauth.settings.apply` is implemented for focused selected-module settings edits. It uses the active selected module or an explicit module target, infers module kind or diagram renderer where possible, and maps requests to `module.settings.update` and `diagram.settings.update` without raw low-level action JSON.
- Successful `mauth.author.ensureSolutions` and `mauth.author.adjustResponseSpaces` calls can finish locally in the frontend. Diagram-bearing `mauth.question.upsert` and `mauth.author.addDiagram` calls now return a compact semantic-review packet instead of immediately ending the chat: the provider compares the teacher request, focused question text, and actual diagram summary, repairs mismatches once with a focused high-level tool, and only then reports success. Failed calls still go through the provider repair loop. The first graph/question semantic inspector now blocks straight-line prompts paired with nonlinear `graph2d` functions after commit so the assistant repairs them before saying done.
...
- `apps/web/src/lib/mauthAssistantFileTools.ts` exposes the first chatbox-ready file tool surface: describe file tools, list/open/save/save-as project files, create folders, duplicate/rename/move/delete files and folders, and list/restore versions through an injected project-file driver. Current-save can omit path/name when the adapter supplies an active project file path and loaded revision.
- Explicit one-step file maintenance prompts now stay on native zero-token routes when the path/name is clear, including create folder, duplicate, rename, move, delete, version history, and version restore. These still execute through `mauth.files.*` in the frontend adapter, so validation, active-file updates, revisions, and visible tool-result handling remain centralized.
- `apps/web/src/lib/mauthAssistantAdapter.ts` is the host boundary for future chatbox UI/provider work. It dispatches document/file tool calls, commits accepted document edits through a host callback, parses opened files through a host callback, serialises the current document for save/save-as, resolves current-save from the active file path/revision, and returns changed ids/paths for user-visible logs.
...
- Right-click `Use in assistant` now sets a first-class active target card instead of pasting `@mauth[...]` reference plumbing into the request field. The assistant can also reuse the current selection with `Use selected`, target cards can jump back to the module, provider/tool replies can keep the same visible target context, and `Copy for assistant` remains available for manual target references.
- The assistant panel now surfaces local tool-result state explicitly: committed/completed, preflight failed with no commit, committed but needs repair, or committed but needs review. Failed and incomplete local states include a `Try repair` or `Review result` action that prepares the next targeted prompt without automatically spending another provider call. Provider final text is suppressed during tool turns so it cannot hide a failed or repair-required local Mauth action.
- Assistant chat state is now keyed by active project file path, so opening a different Mauth file swaps to that file's own conversation instead of leaking stale prompt history across tests. Unsaved documents share a temporary unsaved conversation until saved.
...
- The backend now offers direct provider tools for focused authoring: `mauth_question_upsert`, `mauth_convert_source_question`, `mauth_make_diagram_for_question`, `mauth_write_solutions_for_questions`, `mauth_write_all_solutions`, `mauth_check_document_layout`, `mauth_author_adjust_response_spaces`, `mauth_fix_question_formatting`, and `mauth_update_selected_settings`. They map to the frontend's high-level adapter actions and should be preferred over low-level action batches for single-question writing, source/screenshot conversion, diagram follow-ups, compact solution requests, whole-test solution keys, document-wide layout checks, focused answer-space edits, formatting-only edits, and selected module/diagram settings.
- The backend now narrows provider tool exposure for focused single-question prompts. A request such as "add the diagram to Question 1" exposes only `mauth_make_diagram_for_question`, which reduces tool-schema tokens and makes long expensive tool loops less likely.
- Assistant-facing guidance now exposes first-class `geometry2d` for source-faithful no-axis 2D geometry sketches, and preview summaries report geometry point/segment/arc/angle/decoration counts so semantic review can inspect these diagrams without another paid vision pass. Geometry markers support both declared segment/angle ids and direct point references through `pointPairs`, `anglePoints`, and `points`, so common exam markings do not need hidden helper primitives.
...
- Assistant-authored Penrose-family diagram commits now run native Substance validation plus renderer preflight before mutating the document. Known bad predicates and syntax such as `LabelsPoint`, `SegmentLength`, `OppositeRays`, `Collinear`, `Connect`, raw TeX inside `LabelsAngle(...)`, non-parenthesized predicate calls, `LabelSegment`, raw `Ray(...)`, and bad right-angle/perpendicular arity are rejected with repairable `validationIssues`; changed `geometricConstruction`, `setDiagram`, and `network` blocks are then posted to `/api/diagram/penrose`. If rendering fails, the broken diagram is not committed. Changed Penrose circle-theorem diagrams also run a conservative semantic preflight so valid-but-wrong tangent/chord/circle-membership diagrams are blocked before commit.
...
- High-level assistant edits now have the first automatic post-edit repair loop. After `mauth.author.addDiagram`, a question replacement that includes diagrams or solution/response-space payloads, `mauth.author.ensureSolutions`, `mauth.author.adjustResponseSpaces`, `mauth.format.apply`, or `mauth.settings.apply` commits, the adapter runs `mauth.preview.inspect`; repairable diagram, solution-slot, response-space, formatting-layout, and page-overflow warnings are returned as failed tool output with `validationIssues`, including the target `diagramId` or answer-space id where available, so the provider gets one chance to repair before any success message is shown.
- The post-edit loop now waits for the browser preview to repaint when the preview is mounted, then runs `mauth.preview.inspect` with fresh rendered metrics. This lets the assistant catch painted failures such as failed render placeholders, likely label collisions, clipped diagrams, page overflow warnings, solution-space overflow, missing student answer spaces, hidden mark total mismatches, visible mark notes, and missing L-shaped diagram/answer-space outlines before reporting success.
- High-level diagram edits now include a model-level semantic review stage after deterministic checks pass. The adapter sends the provider a focused, compact artifact summary of the edited question and diagram payload; the provider must reason about whether the diagram actually matches the question rather than relying only on structural validation. This is the preferred path for issues like a straight-line-intersection question receiving a quadratic graph.
- The shared diagram inspection layer now has first targeted semantic checks for more native renderers: `statsChart` manual probabilities must be normalised and use the right probability/relative-frequency mode, `vector2d` prompts must include named coordinate vectors or source-ray metadata, `setDiagram` prompts asking for shading/counts must include shaded regions or numeric count labels, `graph2d` prompts can warn about missing tangent/region/asymptote features, and scalar-product source ray diagrams must include requested labels, angle/right-angle markers, and hidden axes/grid.
- Backend repair continuations keep the same direct authoring tool where possible, so precise failures from `mauth_author_add_diagram`, `mauth_question_upsert`, and `mauth_author_ensure_solutions` are retried on the compact tool surface instead of falling back to broad `mauth_tool` calls.
...
- `pnpm eval:assistant:live` runs a small paid live-provider eval against the configured OpenAI model. `pnpm eval:assistant:live:core` is the standard bounded provider regression suite and currently checks circle-geometry question authoring, Penrose diagram follow-up, diagram-preserving mark edits, diagram-preserving wording rewrites, multipart probability authoring, and a Year 9 linear-intersection graph question that must append Question 2 and use two straight-line `graph2d` functions. `pnpm eval:assistant:live:diagrams` checks renderer selection for coordinate function graphs, Venn/set diagrams, relative-frequency column charts, and coordinate vector diagrams. `pnpm eval:assistant:live:repair` checks the one-retry repair loop by simulating wrong-renderer validation failures and expecting corrected native diagram graphConfigs. `pnpm eval:assistant:live:attachments` checks attachment conversion, including a screenshot scalar-product fixture that requires a renderable native diagram, structured part prompts, correct marks, and no unsolicited solutions. `pnpm eval:assistant:live:confidence` is the bounded paid confidence gate for the highest-value workflows before relying on the assistant: whole-test solution writing through `mauth.solutions.writeAll`, broad `mauth.layout.check`, and one repair round after layout warnings. `pnpm eval:assistant:live:all` runs every focused live case, including tangent-parallel-chord routing, focused solution creation, and repair cases. Use live evals deliberately because they call the real configured model.
...
- `pnpm smoke:assistant:self` runs the cheap deterministic assistant rehearsal suite. It simulates Codex-style assistant choices with real Mauth tools and checks the resulting document for the current high-risk behaviours: mark edits preserving diagrams, diagram follow-ups using native Penrose Substance, wrong renderer choices being rejected, omitted diagrams being preserved, explicit diagram removal only when requested, hidden mark ticks, multipart and whole-test solution targeting, layout-check warnings for missing spaces/oversized diagrams, malformed diagram rejection, raw visible-solution rejection, and validation after authoring.
...
- Automatic post-edit repair loop:
  - Baseline implemented for high-level diagram, solution, and response-space edits: edit -> commit through history/autosave -> wait for browser preview paint when mounted -> preview inspect with fresh rendered metrics -> return repairable warnings once -> only then report success.
  - Diagram edits now add a semantic-review step after deterministic checks pass: the model reviews the compact question/diagram artifact and repairs mismatches before reporting success.
  - Broad layout/print-check prompts now run `mauth.layout.check` with `autoRepair: true`, repairing safe deterministic issues once before reporting remaining issues. Rendered page overflow is repaired only when browser metrics identify one overflowing student space target; ambiguous page overflow remains a model-backed repair task.
  - Next: expand automatic layout repair to additional unambiguous rendered overflow targets, such as a single editable oversized diagram, only when fresh preview metrics can verify the result.
- More diagram-specific inspectors:
  - Baseline implemented for `graph2d`, `vector2d`, `statsChart`, `setDiagram`, scalar-product vector2d source ray diagrams, rendered preview failures, and Penrose circle-geometry semantics.
  - Next: deepen these inspectors with richer renderer output, graph extent/axis-fit checks, Plotly/JSXGraph label placement, 3D camera/label checks, and more geometry-specific theorem checks.
- More high-level native tools:
  - Baseline implemented: one-question authoring/replacement/appending, source-question conversion, focused diagram add/replace, solution writing/mark repair, whole-test solution writing, document-wide layout checks, response-space adjustment, and safe formatting operations.
  - Add more compact teacher-task tools so the assistant does not hand-build huge action JSON: repair all solution spaces, convert whole source pages/papers, apply exam front matter, and repair diagram/layout by id from inspection output.
  - Keep these tools mapped to normal Mauth actions/history/autosave rather than giving the model raw app or filesystem control.
- Better evals:
  - Grow both `pnpm smoke:assistant:self` and paid live evals from real teacher prompts: screenshot conversion, PDF conversion, solution writing, mark edits, diagram repairs, layout repair, and file workflows.
  - Run `pnpm eval:assistant:live:confidence` before trusting broad solution/layout assistant changes; if it fails, add or tighten a no-cost self-smoke case before patching prompts.
  - Every repeat assistant failure should become a deterministic self-smoke case before spending on live model tests.
- Context/cost tightening:
  ...
  - Add an attachment pre-processing pipeline before broad screenshot/PDF source conversion: convert only needed PDF pages; remove blank/duplicate/cover/answer-key pages unless requested; crop scanner borders and blank margins; downscale printed maths pages to the lowest readable resolution; grayscale and clean contrast/noise when it preserves meaning; split large pages into question/diagram/table/solution crops; use low-detail/image triage for classification and page selection; and reserve high-detail vision input for small crops where text, diagrams, handwriting, or layout actually matter. For source conversion and marking, prefer `OCR/math OCR + targeted image crops + native Mauth reconstruction` over sending whole high-resolution pages directly to the model.
...
- Add renderer-level diagram inspection and repair after payload validation. A first shared diagram inspection layer is implemented in `mauth.preview.inspect` and assistant commit preflight: it reports expected-renderer mismatches, missing image sources, missing scalar-product vector labels/angle markers/hidden-axis requirements, rendered diagram failures, likely label collisions, clipping/page-fit warnings when preview metrics are mounted, and conservative Penrose circle-geometry semantic warnings. Remaining work includes graph extent checks, richer visual screenshot/metadata inspection, pedagogical correctness checks for more Penrose families, Plotly/JSXGraph output checks, and print/output fit checks.
- Keep hardening automatic action-repair loops so bad model output is repaired and retried internally instead of surfacing raw tool errors to the teacher. The assistant panel now exposes unavoidable preflight/repair/review states in plain language when the local tool result is not complete.
...
- Extend the implemented `mauth.preview.inspect` selected-context tool with remaining browser state that is not structural yet: file path, solution visibility, print mode, and more diagram-specific semantic checks. Browser-rendered page occupancy, selected-anchor bounding boxes, diagram render failures, solution-slot fit, L-shaped response-space metrics, and first-pass Penrose circle-geometry semantic warnings are now exposed when the preview pane is mounted.
- Implemented high-level artifact answer surfaces for assistant-authored questions: `answerSurface: "diagram"` and `answerSurface: "table"` pair blank/partial student diagrams or tables with completed solution diagrams or tables in the same replacement slot, with red solution ticks rendered beside the completed surface. Next improvement is to expose manual editor controls for converting an existing table/diagram into a paired answer surface without using the assistant.
- Expand `mauth.layout.check` with renderer-level page count, page occupancy, selected-anchor bounding boxes, solution-slot fit, print visibility, and print/PDF assumptions.
...
- Add more high-level assistant workflows built on the action layer: repair solution spaces, add/repair more diagram families, run a print check, combine tests, and generate marking-key variants. `mauth.question.upsert`, `mauth.author.addDiagram`, `mauth.author.ensureSolutions`, `mauth.solutions.writeAll`, and `mauth.layout.check` are the patterns to follow for compact teacher-facing authoring requests.
...
- Move module settings into a right-side inspector panel, similar to Apple-style inspectors. Keep each editor module focused on its main content and immediate editing surface, and put secondary controls such as alignment, visibility, sizing, diagram/chart options, labels, presets, spacing, and advanced metadata in the inspector for the selected module/question/part. The inspector should be selection-scoped: show only the settings for the currently selected module, question, part, subpart, or diagram object, and keep unrelated module settings hidden.
...
- Keep `vector2d` as the coordinate-accurate vector tool with column-vector labels and cleaned graph axes; keep `network` as the schematic network tool.
