...
You do not need the built-in assistant or a published API key to use Mauth with an external coding agent. The repo is set up so Codex, Cursor, Claude Code, or another local tool can work directly against the source tree.

Use this bootstrap:

1. Read `AGENTS.md` and `docs/local-ai-workflow.md`, then `docs/mauth-actions.md`, `docs/ai-brains.md`, and `docs/ai-chatbox-readiness.md`.
2. Work inside `math-app/` for source changes.
3. Keep generated artifacts, OCR crops, eval outputs, and one-off scripts in the sibling `../mauth-workbench/` folder, or another directory outside the app tree.
4. Keep secrets in local `.env` files only and do not commit them.

The action layer, validators, and brain docs are the canonical contract for both internal and external AI workflows.

## In-App Assistant Provider
