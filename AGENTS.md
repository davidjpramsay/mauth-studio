...
The editor supports several open documents in one application window. Each tab owns its document snapshot, independent undo/redo history, navigation position, project/folder identity, file path, loaded revision, dirty/conflict state, and save baseline. Only the active tab drives editor/preview/print state and active-file synchronization. Opening another file or creating a document adds or activates a tab; closing one applies the normal save/discard/cancel guard to that tab only. The recoverable tab set is stored separately in `autosave/open-documents.json` and is still recovery state, not a project-file save. Agent snapshots expose `activeDocumentId` and `openDocuments`; use the optional `documentId` on snapshot, preview, apply, and validation operations instead of guessing the selected tab from browser UI.

The packaged macOS app exports `au.edu.acc.mauth-studio.document`, a dedicated `.mauth` icon, and sandboxed native Quick Look thumbnail and preview extensions from `native/MauthQuickLook`. Keep those extensions read-only and limited to stable JSON document metadata. They must not mutate files, depend on the running API, resolve autosave/revision state, or embed generated PDF/preview caches in `.mauth`. Rebuild them through `pnpm macos:build:document-icon` and `pnpm macos:build:quicklook`; preserve the explicit signing and verification order in the macOS packaging scripts.
