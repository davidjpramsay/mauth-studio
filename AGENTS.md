...
Product direction: Mauth must be a complete teacher-controlled app without AI and a first-class structured authoring surface for Codex/Claude-style external agents. New features, schemas, editor actions, validation, and docs should assume an agent may create, inspect, and edit through the local bridge while the teacher reviews in the same UI. Prefer explicit document state, deterministic validators, reversible actions, stable labels, and small focused controls over behavior that exists only in hidden UI state.

The old provider-backed chat panel is not the product path. Normal users install the signed app and optionally connect an agent through **Help > Set Up Codex or Claude...**; they do not need a setup prompt or repository checkout. Treat this file plus `docs/current-state.md`, `docs/architecture.md`, `docs/local-ai-workflow.md`, `docs/agent-bridge.md`, `docs/mauth-actions.md`, and `docs/ai-brains.md` as the repository operating contract for Codex, Cursor, Claude Code, and other development agents.
...
The standalone app owns its dynamic local API port and child service; no Terminal windows need to remain open. Release builds include a self-contained MCP connector under the app's `Contents/Resources/agent` directory. **Help > Set Up Codex or Claude…** exposes one-time client setup commands without exposing the token. Repository commands such as `pnpm agent:doctor`, `pnpm agent:mcp`, and `pnpm smoke:agent-connector` remain development and diagnostics tools. Both connector paths discover a running app through its private runtime manifest. The packaged bridge is authenticated with a random per-launch token stored only in that mode-`0600` manifest; do not print, persist, or copy the token into source files. Use `pnpm desktop:dev` for the platform-neutral Electron development shell; `pnpm macos:dev` remains its compatibility alias. It owns separate dynamic API and Vite ports: React/CSS edits use Vite HMR and watched API source reloads through Uvicorn, while edits to Electron main-process or packaging files require restarting the shell. `pnpm dev:status` reports either a running packaged app or the fixed-port development runtime. `pnpm dev:stop` stops only fixed-port development servers; quit the packaged app normally with Command-Q or **Mauth Studio > Quit Mauth Studio**.

Keep desktop runtime paths, helper executable names, and native dialogs behind `desktop/platform-paths.mjs` and narrow preload IPC. The web editor and FastAPI services must not assume Finder, AppleScript, `~/Library`, Unix virtual-environment paths, or shell-specific package-manager shims. macOS packaging, Quick Look, Finder registration, signing, notarization, and updater metadata remain explicit macOS adapters until another platform has its own complete package and test matrix.

The standalone Electron app exposes runtime diagnostics and appearance through **View > System Status…** and **View > Toggle Light/Dark Mode**, and solution validation through **Tools > Check Solutions…**. These commands travel through narrow preload events to the existing renderer controllers. The active Student/Solutions toggle already communicates the authoring layer, so Electron does not need a separate Layers badge. Keep browser-only toolbar fallbacks for lower-level web development, but do not restore these auxiliary controls to the macOS toolbar.
...
The app stores authored documents as visible local files under `~/Documents/Mauth/Documents` by default or another teacher-selected folder. New structured documents use the `.mauth` extension and contain versioned JSON with `format: "mauth-studio-document"`; `.test.json` remains read-compatible but is not the default for new saves. `.mauth.md` is the separate text-based Mauthdown exchange format. Preserve an existing file's extension during ordinary saves, renames, and duplicates rather than silently migrating teacher files. On macOS, shared app state, autosave, reusable logo assets, and remembered-folder identity live under `~/Library/Application Support/Mauth Studio/storage`; project metadata and versions for a selected external folder live in that folder's `.mauth` directory. Browser localStorage is only a fallback cache. Treat both the visible workspace and legacy `storage/` as user data, not source code. Logos are not built in: starter logos are editable seed data added once for new or migrated browsers, and the shared logo library is independent of any one saved test. Logo reconciliation may remove a legacy `Logo Only` placeholder only when the exact same image is already retained as a normally named logo; remap embedded legacy ids to that retained asset before rendering an old document. Project/file storage uses revision-aware saves and version snapshots. The Files drawer ZIP backup/import path is the supported portable backup workflow. Treat legacy `storage/tests` as migration input only, while the active state root's `autosave/current-test.json` is recovery state and should not be confused with a saved file.

The editor supports several open documents in one application window. Each tab owns its document snapshot, independent undo/redo history, navigation position, project/folder identity, file path, loaded revision, dirty/conflict state, and save baseline. Only the active tab drives editor/preview/print state and active-file synchronization. Opening another file or creating a document adds or activates a tab; closing one applies the normal save/discard/cancel guard to that tab only. Tabs can be dragged to reorder them without activating or mutating either document, and that order is shared by the overflow menu, recovered tab session, and agent snapshot. The recoverable tab set is stored separately in `autosave/open-documents.json` and is still recovery state, not a project-file save. Agent snapshots expose `activeDocumentId` and `openDocuments`; use the optional `documentId` on snapshot, preview, apply, and validation operations instead of guessing the selected tab from browser UI.

The teacher-facing authoring split is **Content** and **Settings**. Content owns wording, mathematical definitions, coordinates, table values, ordered structure, and add/remove/reorder actions. Settings owns presentation and behavior for the selected block or diagram element, including visibility, colour, line style, sizing, solution-only state, and renderer-specific display choices. Wide workspaces may show both surfaces; compact and overlay workspaces switch between explicit Content and Settings tabs. Give configurable blocks a clear settings button that selects the block and opens Settings. Internal component names may retain `Inspector`, but do not expose that term as the primary teacher-facing label or duplicate Settings controls inline in the normal editor workflow.

Agent document lifecycle uses `mauth_documents_list`, `mauth_document_create`, `mauth_document_open`, and `mauth_document_close`. Create must use the normal template factory and revision-aware project-file save before opening the tab. Open accepts only a relative path returned by list. Close is non-interactive: default `require-clean` refuses dirty state, while `save` and `discard` must be explicit. Never add an MCP lifecycle shortcut that writes a raw `.mauth` file, guesses a path outside the selected folder, bypasses tab state, or waits on a hidden browser dialog.

The packaged macOS app exports `au.edu.acc.mauth-studio.document`, a dedicated PDF-style `.mauth` document icon with the Mauth M, and a sandboxed native Spacebar Quick Look preview extension from `native/MauthQuickLook`. Finder deliberately uses the dedicated icon instead of a generated thumbnail because macOS applies inconsistent framing to third-party thumbnails. Keep the exported type conformed to `public.data`, not `public.json`: the files remain versioned JSON internally, but advertising the generic JSON conformance lets macOS prefer its built-in text thumbnail over Mauth's document artwork. Keep the preview extension read-only and limited to stable JSON document metadata. It must not mutate files, depend on the running API, resolve autosave/revision state, or embed generated PDF/preview caches in `.mauth`. Rebuild the icon and preview through `pnpm macos:build:document-icon` and `pnpm macos:build:quicklook`; preserve the explicit signing and verification order in the macOS packaging scripts.

For macOS releases, `latest-mac.yml` and differential-update data describe the final signed ZIP bytes used by Electron's updater. The notarized DMG is a direct installer: do not advertise it as an updater artifact or publish a DMG blockmap generated before the DMG is signed, notarized, and stapled. Keep the guarded release normalisation and exact remote-asset verification in place.
...
Finder opening must validate the requested document and its hidden index before remembering a new folder. Distinguish an online-only `project.json` from an unavailable assessment, show the requested filename and an explicit offline-folder/Retry action, and never imply that a still-visible recovered draft is the requested file. Queue cold-start opens until draft and tab hydration completes; successful opens add or activate a tab without discarding another document. Keep project requests bound to their owning `documentsPath`, guard delayed reads/saves with tab identity and revision, and keep local recovery backups retrying independently of cloud availability. A timeout on HTTP does not make a forced cloud filesystem read safe.
...
For ordinary tests, exams, and worksheets, store the main stem or shared instruction in the owning question, part, or subpart `text` field. In the editor, the question field is labelled **Question wording**. Reserve `:::text` modules for additional prose whose position among diagrams, tables, choices, columns, or other modules is intentional, or for special blocks such as the final `**End of Test**` marker. Do not leave Question wording blank and create a leading shared text module that merely acts as the stem. Math Notes are the deliberate exception: their top-level question text is the note heading and normal text modules hold the body.

Use `$...$` for inline maths and `$$...$$` for display maths. Write simple prose values as normal text, not LaTeX: use `7%`, `15`, `18 months`, `5.7% p.a.`, and `2024 to 2025` unless the value is part of a real formula, equation, coordinate, variable statement, symbolic table heading, or a mathematical answer option in a choices block. Maths is rendered through MathJax SVG. Inline maths is wrapped with `\displaystyle` by default so it stays in the sentence while fractions and operators use display-style sizing. This is a local wrapper, not MathJax display mode, because MathJax display mode creates block layout. Use full display-size fractions by default throughout printable questions, solutions, tables, and diagram labels. Ordinary `\frac` is sufficient when the surrounding maths style already renders it at display size; in `cases`, arrays, matrices, nested expressions, or any other shrinking context, use `\dfrac{...}{...}` or explicit `\displaystyle`. Use compact `\tfrac` or `\textstyle` fractions only when the teacher explicitly requests them or a genuine layout constraint requires them. TeX still shrinks content inside fraction numerators and denominators; for printable questions, solutions, table cells, and diagram labels, write nested large expressions explicitly, such as `\frac{\displaystyle\binom{n}{r}p^r(1-p)^{n-r}}{\displaystyle\sum_{x=0}^{n} ...}`. Markdown `**bold**`, `*italic*`, and `***bold italic***` are supported in text blocks. Table cells use the same inline maths and formatting renderer.

In solution copies, use the generous student answer space for readable vertical working: usually one mathematical step or conclusion per line. The shared renderer provides enough vertical clearance for display-size inline fractions on adjacent lines; do not insert blank lines just to keep ordinary worked steps apart. Put each hidden `[[marks:1]]` tick on the exact worked line, answer line, completed table, or completed diagram that earns that mark. For ordinary multi-step calculations, prefer several one-mark ticks over one dense final line with `[[marks:2]]` or `[[marks:3]]`.
...
When a standard school test needs a supplied formula or reference page, store it in `frontMatter.formulaSheet` with `enabled`, `title`, and `body`. The renderer places one unnumbered A4 page immediately after the opening title page and before Question 1 in Student and Solutions copies, without changing marks or numbering. In the human editor, an enabled sheet is one fixed **F** item immediately after the opening **T** in the mini TOC; **F+** creates it, **F** selects its dedicated Content editor and preview page, and it is not draggable among questions. Keep the title-page surface limited to inclusion status rather than duplicating the formula fields there. Use blank lines to separate compact two-column sections and the normal dollar-delimited MathJax syntax. Do not fake the page as a zero-mark question, section heading, or ordinary question text module.

Investigation documents use `formattingConfig.id = "investigation"` and intentionally contain no questions, parts, solution slots, or ordinary section markers. Keep structured student pages, text sections, diagrams, general criterion headings, student guidance, and detailed teacher allocation rows under `frontMatter.investigation`. `studentPages` is the source of truth for the brief: each page has a stable id, editable heading, and ordered `sections`; each text section has its own id, optional heading, and body. The legacy `taskTitle`, `taskBody`, `taskBodyContinuation`, and numeric diagram `page` fields remain mirrored for older documents and app builds. Student mode starts with one standard-test title page ordered as title identity, Name/Result row, then the first page's content. Every later student page repeats compact assessment context only; do not repeat the logo, full title page, student row, or administration grid. Put general marking guidance on the final student page. Store investigation diagrams in `diagrams` with an explicit `pageId`, caption, alignment, and normal shared `GraphConfig`, so they use the existing diagram renderers instead of pasted screenshots or fake questions. Keep the narrow mini TOC grouped at page level: **T** for investigation identity, one **P** per student page, and **R** for the teacher rubric. Text sections and diagrams remain nested under their page in the collapsible full navigator instead of consuming separate rail icons. Provide **P+** as the rail control for creating student pages. The rubric always exists, so **R** turns on Teacher mode, opens its editor, and jumps the preview to the rubric; create, reorder, and remove criteria and their performance levels or mark allocations inside that editor. Add text sections from the selected page's own editor. Teacher mode retains every student page and then adds as many compact rubric pages as the detailed criteria need; repeat the rubric context, title, and column headings, but show the final total only on the last rubric page. Treat those shared criteria as the source of truth: edit a heading or guidance once and do not duplicate the rubric as ordinary prose, fake questions, or a separate solutions document. Use `scoringMode: "additive"` when a criterion earns several independent allocations and derive its marks by summing those rows. Use `scoringMode: "holistic"` for alternative performance levels such as 4, 3, 2, and 1, and derive the criterion marks from the highest available level rather than adding the alternatives.
...
When students complete a supplied diagram, its drawing area is already the answer surface. Default to only one unruled line beneath it for a compact marking note; do not add a second large written-response area unless calculation or explanation is requested. Redistribute freed page capacity among the other written-response parts in proportion to their marks, using conservative whole lines and preserving teacher overrides. Verify both Student and Solutions copies. For a completed diagram that already shows the answer, use one concise marking-summary line rather than repeating its values as several prose steps. A single weighted hidden tick such as [[marks:3]] is appropriate on this compact line when it represents the diagram marks; preserve the full allocation and any genuinely required written reasoning. Keep the adjacent student Space at one unruled line when the summary fits, and render-check the pair.

When an agent formats any standard test or exam, this is a required system-wide authoring gate: inspect and adjust spare working space page by page. Share the usable spare line capacity among the existing answer spaces on that page in proportion to the marks represented on those spaces; if marks are unavailable, use equal judgement. Use conservative whole-line counts, conceptually flooring proportional shares so the edits cannot exceed the observed capacity. Leave any uncertain remainder at the page bottom and preserve teacher overrides. A standard test or exam should have one question per printed page wherever it can fit legibly; if a split is necessary, split only before a part or subpart with `pageBreakBefore`. Render and verify fresh Student and Solutions copies after the explicit `Space` edits, and do not call the document ready until both copies pass. The renderer does not automatically infer this proportional allocation.
...
Use **Annotations** as the consistent teacher-facing category for mathematical marks added to existing diagram geometry: free or segment labels, component guides, equal-length/equal-angle ticks, angle arcs, and right-angle/perpendicular squares. Keep structural objects under their natural names such as Points, Segments, Angles, Dimensions, Functions, Faces, and Solids. Saved schemas retain their renderer-specific keys such as `features`, `decorations`, `angleMarkers`, `annotations`, and `dimensions`; do not migrate document data merely to match the shared UI label.

For `graph3d`, define perpendicular/right-angle relationships in one top-level **Annotations** section after **Dimensions**. Each annotation row identifies its owning dimension and offers only connected dimensions or compatible perpendicular faces under **Perpendicular to**. Do not bury this relationship inside every dimension card or duplicate it in Settings; the saved relation remains `rightAngleWith` on the owning `data.dimensions` entry. Dimension targets use the other dimension's stable id; face targets use `face:<face-id>` so the renderer can distinguish a line-to-plane relationship.
...
For copied `graph2d` and `vector2d` coordinate graphs, default to major grid lines only. Use `showMinorGrid: true` only when the source visibly uses minor grid lines, small squares, or fractional grid spacing; do not add minor grid lines just to make a graph look more detailed. Axis number labels should align to the major grid interval. The x-axis number step follows `gridMajorStepX`/`gridMajorStep`, and the y-axis number step follows `gridMajorStepY`/`gridMajorStep`, even when the grid is hidden. Only set `axisLabelIntervalMode: "manual"` with a different `axisLabelStepX`/`axisLabelStepY` when the source intentionally labels a different interval or a teacher asks for custom labels. Use the renderer-owned MathJax number labels for ordinary numeric scales; do not recreate them as coordinate-offset free labels. When one axis needs custom symbolic ticks, use `showXAxisNumbers` and `showYAxisNumbers` to hide only that axis's automatic numbers while retaining the other axis's aligned numeric labels. The legacy `showAxisNumbers` remains the shared fallback. Show required scales directly with numeric or symbolic axis labels instead of describing ordinary grid spacing in the question prose. Keep a written scale instruction only when the graph cannot communicate the intended convention clearly. Keep every axis number or symbolic tick label fully clear of the axis stroke. Renderer-owned numbers belong outside the axis. Put y-axis numbers on the left by default, including custom free-label scales, unless the source or a deliberate layout constraint requires the right side. When symbolic values such as `$\frac{\pi}{4}$` are authored as free `label` features, place x-axis labels clearly below or deliberately above the line, with a visible gap. Increase the offset for stacked fractions and render-check that no label bounding box touches or crosses the axis or an arrowhead.
...
For `graph2d` diagrams whose meaning depends on visual angle, tangent, perpendicular, shortest-distance, or circle geometry, set `equalScale: true`. Choose the graph bounds and `widthPx`/`heightPx` together so the important construction uses the available space at one equal x/y unit scale; do not rely on a very wide box that makes a right angle, slope, or distance relationship look distorted or tiny.

For `graph3d` joined curved solids, author the component solids with exactly matching shared geometry. A cone base centre and radius must equal the spherical cap's cut-circle centre and radius, with their axes pointing in opposite directions. Use `renderStyle: "surface"` on both components when the object must read as a true 3D composite: Mauth combines exact joined pairs into one opaque, depth-ordered face mesh whose cone and cap share the same seam vertices, suppresses the internal shared bases, draws the shared seam once, and derives one camera-live silhouette from the complete projected mesh. Keep every surface face wound outwards. Do not overlay independent cone or cap profile curves on a joined surface because JSXGraph 3D curves do not participate in the face mesh's hidden-line removal and can appear through the wrong component when the view tilts. Use `outline` only for a deliberately unfilled textbook sketch. The outline renderer still treats an exact cone/spherical-cap match as one composite solid: it draws the shared circular seam once and clips the sphere silhouette to the exposed cap so hidden hemisphere curves do not pass through the cone. Do not approximate a joined solid with independent overlapping outlines or freehand projected curves.
...
Normal and density `statsChart` surfaces use structured `data.regions` for shaded areas under the base curve. Each region has a stable id, `mode` (`between`, `leftTail`, `rightTail`, or `outside`), the applicable `lower`/`upper` bounds, fill styling, visibility, and optional `solutionOnly` state. Render the fill without vertical boundary strokes unless a separate annotation is explicitly requested. Agents should upsert, update, or delete one region through the `diagram.settings.update` statsChart region target rather than converting a statistical curve to `graph2d`. When a source does not prescribe exact horizontal bounds, give a normal curve enough domain beyond three standard deviations for its tails to approach the axis before the frame edge; new charts default to approximately `mean ± 3.2 standard deviations`. Preserve deliberate source ranges. Statistics-chart axis numbers and their short outward tick marks are renderer-owned features; never recreate either as individual labels or line objects. Render-check the curve, ticks, numbers, margins, and clip allowance at print scale.
...
Every authored `geometry2d` point, segment, arc, and angle label is independently draggable in the live preview. Persist the moved position in that primitive's `labelX` and `labelY` fields without moving the underlying point, side, arc, or angle. Keep the same coordinates editable through the primitive Inspector and through `diagram.settings.update`. Unlabelled construction markers have no separate drag target.
...
Authored `graph3d` points, segments, faces, and solid surfaces are static geometry. Only the camera and explicitly draggable labels should respond to pointer drags. Keep JSXGraph's generated face polygon, hidden vertices, borders, and 2D projection fixed and non-draggable so a face cannot translate temporarily and then snap back during a projection update.

For school measurement `graph3d` diagrams, use a clean textbook object rather than a coordinate scene. Set top-level `showAxes: false`, hide helper points, and use only the necessary prism edges/faces or curved-solid silhouette. Outline cylinders and cones should show their end curve(s) and the two live camera-facing outside generators; outline spheres should keep one live camera-facing silhouette rather than fixed coordinate great circles. A sphere's orthographic silhouette is correctly circular at every camera angle. Use `renderStyle: "surface"` when the sphere needs stronger spatial cues: Mauth adds a light projected fill and restrained great-circle guides; use `"outline"` for the clean circle-and-radius textbook convention. Curved-solid silhouette generators, sphere outlines, and screen-offset labels must recompute throughout pointer rotation and must not wait for the saved `metadata.view3d` update on release. Every graph3d projection uses a 1:1 horizontal/vertical board scale and expands the authored x/y/z ranges about their existing centres to one equal-span cube before rendering. Here 1:1 means equal screen scale in both directions, not a square frame: `widthPx` and `heightPx` independently control the rectangular page-layout frame and its spare margin without stretching the solid. Use `metadata.view3d.zoom` (the teacher-facing **Object size (%)** control) to enlarge or reduce geometry inside that frame while retaining the equal scale; do not inflate the frame or use unequal ranges just to zoom an object. Use black dimension labels and guides unless the source deliberately uses another colour. When a visible solid edge already shows the measured length, use a dimension on that edge with `display: "label"`; render only the value beside the edge rather than duplicating it with a bracket. When there is no suitable visible outside edge, such as for a radius, diameter, central axis, or perpendicular height, use `display: "guide"` for one dashed construction line without terminal ticks. Use `labelOffsetPx` to tune the rotation-safe screen-space gap. Do not use fixed `labelPosition` coordinates for new label or guide annotations; preserve `display: "bracket"` only for legacy or deliberately fixed-view diagrams. Orient a prism so its length dimension visibly follows the extrusion. Tune `metadata.view3d`, range centres, frame dimensions, and pixel margins together, then render-check the Student copy during a held pointer drag and again after release; the two projections must not snap.

When two connected `graph3d` dimensions represent perpendicular guides, store `rightAngleWith` on one dimension as the stable id of the other. When a perpendicular height meets a polygon face, store `rightAngleWith: "face:<face-id>"` on the height dimension. Mauth verifies that the dimension endpoint lies on the face and that the line is perpendicular to its plane, then derives a small in-plane arm and renders a camera-aware square that stays attached while the view rotates. At a face vertex, the in-plane arm follows the internal corner bisector and is drawn explicitly across the face; do not align a line-to-plane marker with a boundary edge merely because that edge is the nearest face geometry. Use the automatic marker size unless a crowded fixed composition needs a positive `rightAngleSize` override. Do not draw the square as unrelated free geometry.

New and unspecified `graph3d` views default to `metadata.view3d.zoom: 1.3`, shown as **Object size 130%**, and the renderer centres its 3D viewport with a restrained, even inset on all four sides. Preserve an existing explicit zoom, including deliberately smaller values for tall or label-heavy solids; use the shared default instead of authoring `zoom: 1` merely out of habit.

Every authored graph3d point, segment, face, and dimension label is independently draggable in the live preview. Keep its geometric anchor attached to the projected 3D element and persist only the teacher's screen-space adjustment as `labelScreenOffsetPx: [dx, dy]` on that element; reapply the same offset after every camera projection. Omit the field for automatic placement. A double-click or the 3D editor's **Reset label** control clears it, and an agent can clear it with `diagram.settings.update` by setting the patch value to `null`. Do not make axis labels independently draggable and do not replace the screen offset with a fixed 3D coordinate.
...
PDF output uses the browser print dialog and Save as PDF from the same generated A4 preview pages shown on screen. Screen preview uses pixel-sized page boxes for visual layout and zoom. Print CSS should use physical paged-media rules: `@page { size: A4; }` owns paper size and must derive its margins from the active document's stored page padding, while each generated app page renders only that matching printable content area. Do not let print fall back to a different fixed margin, because narrower PDF wrapping can create pages that are absent from preview. Do not make print DOM pages full physical-height paper boxes; that can create blank trailing pages in WebKit/Safari. Avoid browser-specific print branches unless a narrow compatibility issue is proven.
