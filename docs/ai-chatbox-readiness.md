...
- The Assistant chat composer accepts pasted, dropped, or file-picker attachments for images/screenshots and PDFs. The frontend limits the current request to four attachments, 10 MB per file, and 20 MB total, shows attachment chips in the chat, and warns that attachments can increase API cost. The backend passes images to the Responses API as `input_image` data URLs and PDFs as `input_file` data URLs attached to the latest user message.
...
- Extend attachment intake beyond current image/PDF provider inputs: Word documents, curriculum snippets, persistent asset storage, explicit document-upload consent, and extraction/caching for large source files.
...
- Add a real screenshot-image live eval with a readable fixture for "make a question from this screenshot"; the PDF-source live eval already exists.
