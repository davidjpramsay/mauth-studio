...
- `pnpm eval:assistant:live` runs a small live provider eval for the focused teacher prompt "write Question 1 with a circle-geometry proof and solution". `pnpm eval:assistant:live:core` is the normal paid regression suite: it covers circle-geometry question writing, Penrose diagram follow-up, mark-allocation edits that must preserve diagrams, wording rewrites that must preserve diagrams, and multipart probability authoring under a default $1.50 safety cap. `pnpm eval:assistant:live:all` runs every live case. These are paid real-provider checks, so use them deliberately.
