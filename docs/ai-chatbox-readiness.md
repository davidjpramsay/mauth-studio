...
- Before assistant-authored Penrose diagrams are committed, the frontend adapter preflight-renders changed `geometricConstruction`, `setDiagram`, and `vectorRelationship` blocks through the same `/api/diagram/penrose` request used by preview. Render failures return repairable `validationIssues` and no document mutation occurs. This catches Substance mistakes such as passing raw TeX directly to `LabelsAngle`; the assistant should repair the `graphConfig` rather than leaving a "Geometry diagram could not render" block in the document.
...
- The frontend tool loop gives failed tool/preflight output one provider repair attempt using the exact validation issues. If the repair also fails, the loop stops with a clean teacher-facing message instead of continuing through expensive failed tool rounds.
