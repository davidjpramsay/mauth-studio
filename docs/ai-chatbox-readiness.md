...
- For scalar-product/vector-ray screenshots with magnitudes and angle markers but no coordinate axes, the assistant should recreate the source as a native `geometricConstruction` diagram. Use supported Penrose Substance, preserve vector labels, magnitude labels, right-angle markers, and numeric angle labels, and wrap the renderer payload as `{ graphConfig: { type: "geometricConstruction", ... } }`. Point/vector endpoint labels should be direct `Label A $\mathbf{a}$` statements, not `LabelsPoint`. Visible right-angle markers should use `RightAngle(pointOnFirstRay, vertex, pointOnSecondRay)`, not `PerpendicularToSegment`.
...
- Repair continuations stay on the same narrow direct authoring tool when possible. A failed `mauth_author_add_diagram`, `mauth_author_replace_question`, or `mauth_author_ensure_solutions` call should receive that same direct tool for the retry instead of reopening the broad wrapper surface.
