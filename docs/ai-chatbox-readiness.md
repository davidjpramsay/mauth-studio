...
- Add a real screenshot-image live eval with a readable fixture for "make a question from this screenshot"; the PDF/Word-source live eval already exists.
- Add a specific screenshot conversion eval for "diagram under the stem, parts under the diagram" that fails if the assistant replaces the diagram with prose or emits empty part prompts.
