...
- The diagram validator rejects those known-invalid Penrose Substance mistakes before apply, then render-preflights changed Penrose diagrams. After render succeeds, assistant commit preflight also runs conservative semantic checks for Penrose circle geometry. Treat returned `validationIssues` as the repair target and retry once with corrected native Substance.
- `mauth.preview.inspect` performs the same conservative semantic checks on Penrose circle geometry after a diagram exists. It does not replace mathematical judgement, but it catches common valid-but-wrong outputs such as a drawn line that is not declared as a tangent, a tangent not parallel to the named chord, a named chord not drawn as a segment, named points not on the same circle, or an auxiliary centre label being shown when the prompt does not name it.
