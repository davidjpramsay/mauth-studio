...
Use `pnpm eval:assistant:list` to print the current machine-readable tool/eval taxonomy, live groups, local groups, and case classifications. If a new failure does not fit one of these request classes, add the smallest new class or high-level tool deliberately; do not hide it as a source-specific prompt rule.
