...
Fresh read-only assistant requests such as "inspect the test", "run validation", broad layout checks, help prompts, and simple clarification prompts should stay on native zero-token routes before provider routing. The backend may return local tool calls for `mauth.document.inspect`, `mauth.validation.run`, or `mauth.layout.check` without requiring an OpenAI key; only repair or authoring follow-ups should spend provider tokens.
