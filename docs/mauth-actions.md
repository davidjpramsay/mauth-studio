...
- `mauth.author.replaceQuestion`: replace one existing question from a compact authoring payload with question text, marks, optional diagrams, student-only answer space, and solution-only solution text. Omitted `diagram`/`diagrams` preserves existing diagrams; use `diagrams: []` or `preserveExistingDiagrams: false` only when intentionally removing diagrams.
