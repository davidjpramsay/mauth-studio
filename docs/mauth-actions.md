...
- For screenshot scalar-product ray diagrams with common-origin vectors, magnitudes, right-angle markers, and angle labels but no axes, use `graphConfig.type: "geometricConstruction"` with supported Penrose Substance. Preserve the labels, angle markers, and opposite collinear rays shown in the source; use `VectorSegment`, `RayFrom`, `LabelsSegment`, `LabelsAngle`, and `RightAngle`. Predicate calls must use parentheses and commas, e.g. `VectorSegment(OA, O, A)`. Label vector endpoints directly with `Label A $\mathbf{a}$`; do not invent `LabelsPoint`. Use `RightAngle(pointOnFirstRay, vertex, pointOnSecondRay)` for the visible marker; do not use `PerpendicularToSegment` for this. To preserve opposite collinear rays, declare a line such as `Line lineAD`, then use `LineThrough(lineAD, A, D)` and `On(O, lineAD)`. Do not use `vectorRelationship`, unsupported `LabelSegment`/`Ray(...)` syntax, raw TeX inside `LabelsAngle(...)`, or invented predicates such as `SegmentLength`, `OppositeRays`, `Collinear`, or `Connect`.
