...
- The diagram validator rejects those known-invalid Penrose Substance mistakes before apply, then render-preflights changed Penrose diagrams. After render succeeds, assistant commit preflight also runs shared diagram inspection for hard failures: wrong renderer, missing image source, missing scalar-product vector labels, and conservative Penrose circle-geometry semantic issues. Treat returned `validationIssues` as the repair target and retry once with corrected native Substance or corrected renderer config.
- `mauth.preview.inspect` performs the same first-pass diagram inspection after a diagram exists. It does not replace mathematical judgement, but it catches common valid-but-wrong outputs such as a drawn line that is not declared as a tangent, a tangent not parallel to the named chord, a named chord not drawn as a segment, named points not on the same circle, an auxiliary centre label being shown when the prompt does not name it, or vector-ray diagrams missing labels for vectors used in scalar products. When browser preview metrics are mounted, it also reports rendered failures such as failed diagram placeholders, likely label collisions, clipped diagrams, page overflows, solution-space overflow, and missing L-shaped diagram/answer-space outlines.
