...
For high-level diagram edits, the frontend adapter performs an automatic post-edit check before reporting success. After `mauth.author.addDiagram` or a question replacement with diagram payloads commits through history/autosave, the adapter runs `mauth.preview.inspect` for that question. If `question.diagrams[].warnings` contains repairable warnings or errors, the adapter returns failed tool output with `validationIssues` and the target `diagramId`. The provider should retry the same high-level diagram tool once, include that `diagramId`, and provide a corrected native `graphConfig`. This is the standard repair path, not a teacher-facing failure.
