...
- High-level assistant diagram edits also run an automatic post-edit preview inspection before success is reported. After the edit commits, the adapter waits for the browser preview to repaint when mounted, then runs `mauth.preview.inspect` with fresh rendered metrics. If it returns repairable diagram or rendered-layout warnings, the adapter returns failed tool output with `validationIssues` and a `diagramId` where available; the assistant should retry the same diagram tool once with that `diagramId` and a corrected native `graphConfig` or corrected adjacent layout so the bad output is repaired, not duplicated.
