...
- Use `equalScale: true` for `graph2d` constructions where tangents, perpendiculars, angles, shortest distances, or circles must look geometrically true. Match the graph bounds and pixel dimensions so the construction remains large enough to read at that 1:1 scale.
...
- Copied coordinate graphs should use major grid lines only by default. Set `showMinorGrid: true` only when the source visibly uses minor grid lines, small squares, or fractional grid spacing. Prefer integer `xMin`/`xMax`/`yMin`/`yMax` bounds for ordinary classroom coordinate graphs; use half-unit or fractional bounds only when the source scale, required point placement, or mathematical feature genuinely needs them. Axis number labels should align to the major grid interval: `gridMajorStepX`/`gridMajorStep` for the x-axis and `gridMajorStepY`/`gridMajorStep` for the y-axis, even when the grid is hidden. Use different manual axis-label steps only when the source intentionally labels a different interval or a teacher asks for custom labels. Keep ordinary numeric ticks renderer-owned so they share the MathJax typeface and remain anchored to the exact grid values; do not reproduce them as shifted free labels. When one axis uses custom symbolic ticks, set `showXAxisNumbers` or `showYAxisNumbers` independently so only the conflicting automatic labels are hidden. Put required scale values on the axes instead of explaining ordinary grid spacing in the question prose; retain prose only when the graph cannot show the convention clearly. Keep all numeric and symbolic tick labels outside the axis stroke with a visible gap. Put y-axis numbers on the left by default, including custom free-label scales, unless the source or a deliberate layout constraint requires the right side. If symbolic ticks such as `$\frac{\pi}{4}$` use free `label` features, offset them farther than single-line values and render-check that no label touches an axis, tick mark, or arrowhead.
...
- Finish a school test with one final question-level text block containing exactly `**End of Test**`. Keep it as the final visible module. The renderer centres it at the bottom of the final question page in Student and Solutions copies, so do not simulate its position with blank lines, a trailing Space block, or manual text padding.
- For an investigation, use `titlePageTemplate: "investigation"` and formatting preset `"investigation"`. Do not create fake questions. Build the student brief with `frontMatter.investigation.studentPages`; give each page stable text sections and assign diagrams by `pageId`. The first page shares the standard-test title identity and Name/Result row; later pages repeat compact assessment context only, and general marking guidance belongs on the final student page. Keep the narrow mini TOC at title, page, and rubric level; expose text sections and diagrams only as nested items in the collapsible full navigator. Use **P+** for a new student page. The rubric always exists, so **R** turns on Teacher mode, opens its editor, and jumps the preview to the rubric; add, reorder, and remove criteria and their performance levels or mark allocations there. Add text sections inside the selected page editor. Teacher mode adds as many compact rubric pages as the detailed criteria need, repeating the rubric context/title/columns and showing the final total only on the last page. Use additive criteria for independent mark allocations and holistic criteria for alternative performance levels; a holistic 4/3/2/1 scale contributes 4 marks, not 10. Derive totals from the structured criteria rather than typing a separate total.
