...
- Assistant decision and authoring changes should be checked against `pnpm smoke:assistant:self` before paid live-provider evals. The self-smoke suite uses realistic teacher prompts and the same Mauth tools as the in-app assistant, then grades the output for diagram preservation, renderer choice, hidden mark ticks, student/solution pairing, and validation safety.
