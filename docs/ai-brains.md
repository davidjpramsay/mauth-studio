...
- High-level assistant diagram edits also run an automatic post-edit preview inspection before success is reported. If `mauth.preview.inspect` returns repairable diagram warnings, the adapter returns failed tool output with `validationIssues` and a `diagramId`; the assistant should retry the same diagram tool once with that `diagramId` and a corrected native `graphConfig` so the bad diagram is replaced, not duplicated.
