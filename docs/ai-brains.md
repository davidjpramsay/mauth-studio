...
- Assistant decision and authoring changes should be checked against `pnpm smoke:assistant:self` before paid live-provider evals. The self-smoke suite uses realistic teacher prompts and the same Mauth tools as the in-app assistant, then grades the output for appending the next missing question, diagram preservation, renderer choice, hidden mark ticks, student/solution pairing, and validation safety.
...
- High-level assistant diagram, solution, and response-space edits also run an automatic post-edit preview inspection before success is reported. After the edit commits, the adapter waits for the browser preview to repaint when mounted, then runs `mauth.preview.inspect` with fresh rendered metrics. If it returns repairable diagram, solution-slot, response-space, or rendered page/layout warnings, the adapter returns failed tool output with `validationIssues` and a target id where available. Retry the matching high-level tool once: use `mauth.author.addDiagram` with the returned `diagramId` for diagram repairs, `mauth.author.ensureSolutions` for solution/tick repairs, or `mauth.author.adjustResponseSpaces` for answer-space, overflow, and L-shaped layout repairs.
...
- Use `mauth.author.replaceQuestion` for writing or replacing one question. It can replace an existing question or append the immediate next missing question.
...
- High-level diagram arguments must use the native wrapper shape `{ graphConfig: { type: ... }, diagramAlign?: ... }`. Do not put renderer fields such as `type`, `data`, or `options` directly on `diagram`, and do not use `config` as a shortcut for `graphConfig`.
...
For local development, treat `pnpm smoke:assistant:self` as the no-cost assistant rehearsal before using the real API. If it fails, fix the Mauth tool contract, validators, or brain rules first; do not spend live-provider calls trying to prompt around deterministic tool defects.
