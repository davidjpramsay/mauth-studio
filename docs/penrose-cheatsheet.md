...
The agent-facing path for schematic theorem geometry should normally start with `penroseGeometryDiagram`, which describes relationships such as points, circles, tangents, segments, angle markers, parallels, and equal lengths. Mauth compiles that semantic payload into this supported Penrose Substance dialect. Use raw `graphConfig.options.substanceSource` only when the builder cannot express the construction. Structured `graphConfig.data` geometry can exist for simple UI controls, but it should not become a second Penrose language.
