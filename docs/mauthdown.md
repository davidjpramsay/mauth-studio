...
For `graph2d`, graph bounds (`xMin`, `xMax`, `yMin`, `yMax`), size (`widthPx`, `heightPx`), axes/grid display flags, `functions`, and `features` are top-level diagram JSON fields. Only renderer-specific data such as `slopeField` belongs under `data`. Do not put `functions` or `features` under `data`, do not put axes/size fields under `options`, and use `domainMin`/`domainMax` plus `color`/`strokeWidth`/`strokeStyle` directly on each function rather than `domain` or `style` wrapper fields. Graph features use `kind`, not `type`, and feature style fields such as `color`, `size`, and `strokeWidth` also live directly on the feature. For shaded regions and loci, define boundary curves/rays in `functions`, then use `region_between_curves`, `region_curve_axis`, or `region_clipped_by_curve` features with function indices and `fillOpacity`; fields such as `expressionTop`, `expressionBottom`, `opacity`, and `fillColor` are not part of the graph2d feature schema. For Argand loci, preserve the source or marking-key argument reference; a locus can combine a shifted circle such as `|z-i| <= 2` with sector bounds such as `Arg(z)`, so do not rewrite the argument expression unless the source does.
