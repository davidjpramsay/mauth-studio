...
Angle and perpendicular markings are controlled by `metadata.vector2d.angleMarkers`. Each marker chooses a `from` vector and a `to` vector; use `rightAngle: true` for a perpendicular marker, or leave it false and supply a label such as `45^\circ` for a normal angle arc. The editor exposes these as manual Annotations controls with marker radius, colour, and optional draggable label position.
