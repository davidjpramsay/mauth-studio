...
The editor's Student/Solutions toggle is also the manual solution-editing mode. In Student mode, newly inserted ordinary modules are shared/student-facing as usual. In Solutions mode, newly inserted text, table, diagram, and columns modules are created as solution-only modules so teachers can author worked solutions, completed tables, and completed diagrams directly without editing raw JSON.
