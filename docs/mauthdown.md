...
Standard tests may include one optional unnumbered reference page under `frontMatter.formulaSheet`: `{ "enabled": true, "title": "Formula Sheet", "body": "..." }`. The page renders immediately after the opening title page and before Question 1 in both Student and Solutions copies. In the human editor it is the fixed **F** document item after the opening **T**, created with **F+** and edited on its own Content surface rather than inside a question. Use blank lines to separate compact sections in `body`; the formula-sheet renderer flows those sections into two columns and supports the normal dollar-delimited MathJax and inline Markdown emphasis. Do not make a zero-mark question or section heading to imitate this page.

Investigation documents use `formattingConfig.id = "investigation"` and keep the whole authored assessment under `frontMatter.investigation`. They do not contain questions, parts, solution slots, or ordinary section markers. The student brief is an ordered `studentPages` array. Each page has a stable `id`, a `title`, and ordered `sections`; each text section has its own `id`, optional `heading`, and `body`. Student mode starts with one standard-test title page ordered as title identity, Name/Result row, then the first student page. Every later student page repeats only compact assessment context and its page heading, not the logo, student row, full title page, or administration grid. General marking guidance appears on the final student page. Structured `diagrams` use `pageId` plus a title, caption, alignment, and the normal shared `GraphConfig`, so graph, geometry, statistics, image, and Penrose renderers remain reusable. The narrow mini TOC stays grouped at **T** investigation identity, **P** student pages, and **R** rubric level. Text sections and diagrams remain nested under their page in the collapsible full navigator instead of becoming separate rail icons. **P+** creates a student page. The rubric always exists, so **R** turns on Teacher mode, opens its editor, and jumps the preview to the rubric; criteria and their performance levels or mark allocations are added, reordered, and removed there. Text sections are also added from the selected page editor. Legacy `taskTitle`, `taskBody`, `taskBodyContinuation`, and numeric diagram `page` values are still read and mirrored for compatibility. Teacher mode renders every student page followed by as many compact teacher-rubric pages as the criteria need. Each rubric page repeats the assessment context, rubric title, and column headings; instructions appear on the first rubric page and the final total appears only on the last. The teacher rubric reuses each criterion's `heading` and `guidance`, then adds its detailed `allocations`. For `scoringMode: "additive"`, the criterion total is the sum of its allocation rows. For `scoringMode: "holistic"`, the rows are alternative performance levels and the criterion total is the highest available level, not the sum of every level.
        ...
        "sections": [
          {
            ...
            "body": "Describe the mathematical problem to be investigated."
          }
        ]
      }
    ],
...
Standard test and exam booklet layout must use one question per printed page wherever the question can fit legibly. Set a question-boundary page break after each question except the final question unless the question cannot fit on one page after reasonable space allocation. Use the question's own part/subpart answer-space blocks to fill most of the available page area, distributing extra working lines across those spaces in proportion to the marks rather than adding one unrelated trailing space block before the page break. If a split is necessary, split only before a part or subpart with `pageBreakBefore`, then freshly verify both Student and Solutions copies.
...
For ordinary tests, exams, and worksheets, store the main stem or shared instruction in the question's `text` field, shown as **Question wording** in the editor. Part and subpart prompts belong in their own `text` fields. Use `:::text` only when prose must be deliberately ordered among diagrams, tables, choices, columns, or other modules, or for a special module such as the final `**End of Test**` marker. A blank Question wording field followed by a leading shared text block is redundant and is reported by document validation. Math Notes are exempt because the top-level question text is the note heading and text modules form the body.
...
Use full display-size fractions by default throughout printable questions, solutions, tables, and diagram labels. Ordinary `\frac` is sufficient when the surrounding maths style already renders it at display size. In `cases`, arrays, matrices, nested expressions, or any other context where TeX would make the fraction small, use `\dfrac{...}{...}` or explicit `\displaystyle`. Use compact `\tfrac` or `\textstyle` fractions only when the teacher explicitly requests compact notation or a genuine layout constraint requires it.
...
For solution copies, add `[[marks:n]]` at the end of a mark-worthy text or equation line. The preview hides the raw annotation and renders `n` large red check marks at the right edge of that line. In the editor, solution-only text blocks provide mark-tick buttons that insert these hidden annotations at the cursor. Do not write visible `[1 mark]`, `(1 mark)`, or `Solution (5 marks)` notes in displayed solution prose. Completed solution tables and diagrams can carry `markTicks` so the red check marks render next to the completed surface instead of a following text line.

Use the generous student answer space for readable vertical working in solution copies. A normal calculation solution should usually have one equation, method step, answer line, or short conclusion per line. Put each hidden `[[marks:1]]` tick on the exact line or completed surface that earns that mark, so the red tick aligns with the awarded work. For ordinary multi-step items, prefer several one-mark tick lines over one dense paragraph ending with `[[marks:2]]` or `[[marks:3]]`. The shared renderer leaves enough vertical clearance for display-size inline fractions on adjacent lines; keep words short and marking-focused, and do not add blank lines merely to separate ordinary worked steps.
...
The editor's Student/Solutions toggle is also the manual solution-editing mode. In Student mode, newly inserted ordinary modules are shared/student-facing as usual. In Solutions mode, newly inserted text, choice list, table, diagram, and columns modules are created as solution-only modules so teachers can author worked solutions and completed surfaces directly without editing raw JSON. Existing shared multiple-choice lists and completion tables are answered in place instead: Solutions mode exposes their structured answer data and surface ticks, while Student mode hides both.

For a shared `graph2d` diagram, Solutions mode changes the **Add feature** action to **Add solution annotation** and the function action to **Add solution function**. New points, labels, segments, tangents, markers, shaded regions, and functions created through those actions default to `solutionOnly: true`, display a Solution badge in the editor, and disappear when Student mode is selected. Solution functions render in solution blue without recolouring shared curves. Student mode also hides features that reference a hidden solution function while preserving the original function indexes. The selected feature or function's **Show in solutions only** checkbox remains the explicit override when an element should instead be shared. Use this in-place function layer for answer curves; use **Complete in solutions** only when the whole graph should differ independently.

For a shared `geometry2d` diagram, Solutions mode applies the same authoring-layer rule to new points, segments, arcs, angles, and construction markers. These primitives default to `solutionOnly: true`, display a Solution badge, disappear from the Student editor and preview, and render in solution blue without recolouring shared construction elements. The selected primitive's **Show in solutions only** inspector checkbox is the explicit manual override. Use this element-level path when the solution adds a small construction, label, angle, or marker to a shared 2D diagram; use **Complete in solutions** when the whole diagram should be independently completed.

For a shared `vector2d` diagram, vectors, segment labels, and angle markers use the same authoring-layer rule. Elements added in Solutions mode default to `solutionOnly: true`, display a Solution badge, disappear from Student editor and preview, and render in solution blue without recolouring shared vectors or annotations. Student mode also suppresses a segment label or angle marker when one of its referenced vectors is solution-only. Use the element's **Show in solutions only** control for deliberate layer changes, and use **Complete in solutions** when the whole vector construction should be independently completed.

For a shared `graph3d` diagram, points, segments, and dimensions use the same authoring-layer rule and can be created directly in the 3D editor. Elements added in Solutions mode default to `solutionOnly: true`, display a Solution badge, disappear from Student editor and preview, and render in solution blue. Existing faces and solids expose the same **Show in solutions only** override. Student preview keeps hidden solution points as `show: false` range inputs so the camera framing does not jump, while suppressing any line, face, or solid that depends on a hidden point. Point, segment, face, and dimension labels can be dragged independently in the live preview; double-click one, or use **Reset label** in the 3D editor, to return it to automatic placement. Use **Complete in solutions** when the whole 3D construction should be independently completed.

For a shared Plotly `statsChart`, use supplemental entries in `data.series` for small answer overlays. Lines, points, line-and-point traces, and bars added in Solutions mode default to `solutionOnly: true`, display a Solution badge, disappear from Student editor and preview, and render in solution blue without recolouring the base chart. Use the series **Show in solutions only** control for deliberate layer changes, and use **Complete in solutions** when the whole chart should be independently completed.

For a shared uploaded `image`, use entries in `data.annotations` for focused labels, ellipses, and arrows. Annotations added in Solutions mode default to `solutionOnly: true`, display a Solution badge, disappear from Student editor and preview, and render in solution blue without recolouring the bitmap or shared annotations. Their positions and dimensions are percentages of the configured image box so they remain aligned when the image is resized. Use the annotation's **Show in solutions only** control for deliberate layer changes, and use **Complete in solutions** only when the whole image should differ independently.

For structured Penrose geometry and network diagrams, Solutions mode creates supported points/nodes and segments/links as `solutionOnly` elements. Student mode removes them and relationships that depend on hidden points; Solutions mode renders only the answer elements and labels in solution blue. Structured two-set and three-set Venn diagrams can similarly mark region labels/values and shading as solution-only without moving the fixed regions. Keep Advanced Substance separate: use **Complete in solutions** for custom constraints, unsupported complex construction predicates, or a substantially different completed Penrose diagram.

A shared diagram containing supported structured `solutionOnly` elements is both the student response surface and the completed solution surface. It may carry `markTicks` directly: Student mode hides the answer elements, Solutions mode colours only those elements blue, and the red ticks render beside the diagram without recolouring the shared surface. Use this path for focused answers on `graph2d`, `geometry2d`, `vector2d`, `graph3d`, `statsChart`, uploaded images, structured Penrose geometry/network, and Venn regions.

**Complete in solutions** creates the editable paired diagram but does not complete the answer by itself. Solution validation compares mathematical content and keeps an untouched copy incomplete. Resizing, recolouring, changing line weight, toggling grids, changing coordinate or chart view ranges, and moving the 3D camera do not count as answer content. Add the required curve, point, label, value, shading, series, vector, construction, or replacement image before the scope can become **Solution ready**.

Blank answer/completion tables are different from given-data tables. An ordinary table with empty body cells for students to complete is one shared response surface. Its answers belong in the sparse `solutionEntries` matrix on that same table; given values remain in `rows`. A table that only gives values or information for the question stays shared without solution entries.

In manual editor mode, a shared table exposes **Complete in solutions** in its own panel header. The action switches to Solutions mode and opens that same root or column-nested table. Blank body cells become answer-entry fields; headings and given values remain read-only. Student mode shows the original blank cells, while Solutions mode substitutes the stored answers.

For completion-table answer surfaces, keep the completed answer on the shared table. In structured JSON, `solutionEntries` parallels body `rows`, may contain a value only where the matching student cell is blank, and stores an empty string everywhere else. Put the awarded red ticks in `markTicks` on that same table. In Mauth actions, use `module.settings.update` with `{ "kind": "table", "solutionEntry": { "row": 0, "column": 1, "value": "6" } }`; body coordinates are zero-based and `null` clears an entry. Do not replace the table answer with only a LaTeX array or duplicate the same marks in `solutionText`.

Only stored answer entries render in solution blue. Row/column labels, given values, and the table grid keep normal styling. Adjacent student-only and solution-only table pairs remain readable for older documents, but new ordinary completion tables should not create that duplicate structure.
...
Graph grids, charts, diagrams, tables, and choice lists can also be response surfaces. If the task is to sketch a graph, label a diagram, shade a region, draw a function, select an option, or complete values directly on a visual surface, the student copy should show the blank or partial surface and the solution copy should show the completed surface in the same position and size. Use shared structured answer data for choices, ordinary completion tables, and focused supported diagram annotations. Use a paired `solutionDiagram` only when the completed diagram must differ independently. No separate large `:::space` block is needed unless the question also asks for written working.

In raw test JSON, solution modules are encoded with `visibility: "solution"` or the legacy `solutionOnly` compatibility field. Ordinary question modules should not be given a copy setting; they stay shared by default. Shared choices use `solutionAnswerIndex`, and shared completion tables use `solutionEntries`. The current `:::table` Mauthdown container describes the student-facing table rows; author or revise its structured solution entries through the editor, JSON schema, or Mauth action layer. `space` blocks are treated as student answer/work space by default.
...
The formatted preview uses a slightly open shared line height for question and solution text so display-size inline fractions on adjacent lines remain visibly separate. Do not force tighter paragraph spacing just to save space or insert blank lines as a local workaround; add or remove answer-space lines deliberately instead.
...
Solution validation runs as agent/editor infrastructure. It scans marked questions, parts, and subparts for student response surfaces, solution modules, matched replacement slots, blank worked-solution text, incomplete shared or legacy paired completion tables, unchanged paired solution diagrams, likely solution-fit problems, and mark totals. It recursively follows content inside columns, including inherited Student/Solutions visibility. Shared selected choices, table entries, and supported structured diagram annotations count as response and solution content. The mark check sums hidden text `[[marks:n]]` annotations and structured choice/table/diagram `markTicks`, then compares the printed solution total with the marks on that question, part, or subpart. A blank solution module or unchanged paired diagram is an editable authoring slot, not proof that the answer has been written. Treat validation as an AI and teacher finish check: after generating or editing solutions, fix validator errors and any avoidable warnings before calling the solution pass complete. The header still shows the current print mode as `Print: Student` or `Print: Solutions`; browser print/PDF output follows that same solution visibility state.

Validator issue cards include quick fixes for common authoring mistakes: add a full student-space/solution slot, add the missing solution after an existing answer space, add the missing answer space before an existing solution, or increase the paired answer-space line count. The same fixes work for blocks inside columns by patching the owning columns module through the structured action layer. Mark-total warnings are deliberately review-only because the teacher must decide which line or completed surface earns each mark.

In Solutions mode, each marked question, part, or subpart header also shows its current solution state. Leaf items expose the relevant quick fix, such as **Add solution**, while parent headers summarize issues in their descendants. **Solution ready** is shown only after the validator finds the required student response surface, complete solution content, and a printed solution-tick total that matches the item marks. These compact controls use the same validator and Mauth action path as the full validation panel.
...
For a multiple-choice answer key, keep the original choices block as one shared student-and-solution response surface. In structured JSON, set `solutionAnswerIndex` on that block to the zero-based chosen option and store its awarded marks in `markTicks`. In Mauth actions, use `module.settings.update` with `{ "kind": "choices", "solutionAnswerIndex": 1 }` to circle the second option, or `null` to clear the selection. Student mode does not render the answer ring or ticks. Solutions mode keeps all unselected choices in normal styling and colours only the circled label blue. Do not create a duplicate list or add a prose sentence underneath just to state the selected option. Existing solutions-only choice copies remain supported for older documents.
...
For a completion-table answer key, keep one shared table. In structured JSON, store answers in its sparse body-row `solutionEntries` matrix and ticks in `markTicks`; in the action layer, set one answer with the focused `module.settings.update` `solutionEntry` setting. The current Mauthdown table container records the student-facing rows, so use the editor, JSON schema, or action layer for this solution metadata. Student mode hides entries and ticks; Solutions mode colours only the substituted answer cells blue.
...
Express an ordinary required graph scale through numeric or symbolic axis labels rather than prose that describes the grid spacing. Keep a written scale explanation only when the graph cannot show the intended convention clearly. Put y-axis numbers on the left by default, including manually authored free-label scales, unless the source or a deliberate layout constraint requires the right side.
...
The editor treats `graph3d` as its own renderer family. It exposes **Frame width**, **Frame height**, and the stored camera values `az`, `el`, and `bank`. The two frame values independently control page-layout space; the renderer keeps equal horizontal and vertical screen scale, so a rectangular frame does not distort the solid. For source 3D coordinate geometry, put named vertices in `data.points` and visible joins in `data.segments`; use `strokeStyle: "dashed"` or `dashed: true` for hidden edges, not a segment `style` field. Preserve source line/ray/vector notation in part text and graph3d segment labels: a source line or main diagonal labelled `BT` or `\overleftrightarrow{BT}` should stay as line notation, not become `\overrightarrow{BT}`. Do not paste PDF-extraction control characters for line symbols or Greek parameters; write `\overleftrightarrow{BT}`, `\lambda`, and `\mu` explicitly. Named angle notation also requires explicit segment rays: the middle letter is the angle vertex, so `\angle DMF` needs both `DM` and `MF` in `data.segments`; if `M` is a midpoint on `EF`, drawing the whole `EF` edge alone does not preserve the `MF` ray, and midpoint-construction segments such as `AF` and `FB` do not preserve it either. Use `data.faces` with `{ points: [...] }` entries for polygon faces on prisms and pyramids, not `{ vertices: [...] }`, and use `data.solids` for curved solids with `kind: "cone"`, `"cylinder"`, `"sphere"`, `"circle"`, or `"sphereCap"`. Cone/cylinder solids use `baseCenter`, `apex` or `topCenter`, and `radius`; spheres/circles use `center` and `radius`, with optional `normal` for circles. Solids accept `renderStyle: "surface"`, `"wireframe"`, or `"outline"` so source diagrams can choose filled surfaces or clean outline-only sketches. Dimension annotations such as height `h` and radius `r` belong in `data.dimensions` and can reference point ids or coordinate triples. Use `{ from, to, label, display: "label", labelOffsetPx? }` when those endpoints are an existing visible edge; Mauth renders only the value beside that edge. Use `display: "guide"` for a radius, diameter, central axis, perpendicular height, or any other measurement that needs a dashed construction line. New label and guide annotations have no terminal ticks and recompute their screen-space label gap as the teacher rotates the solid. When two dimensions share an endpoint and should show perpendicularity, set `rightAngleWith` on one dimension to the stable id of the other. When a perpendicular dimension meets a polygon face, set `rightAngleWith: "face:<face-id>"`; Mauth verifies the shared foot and line-to-plane relationship before deriving the in-plane arm. At a face vertex, that arm follows the internal corner bisector and is explicitly drawn across the face instead of coinciding with a boundary edge. Both marker forms remain attached during camera rotation, and optional positive `rightAngleSize` overrides the automatic size. Every labelled point, segment, face, and dimension may also store `labelScreenOffsetPx: [dx, dy]`; the geometric anchor reprojects with the solid and the saved pixel adjustment is then reapplied. Omit the field for automatic placement. In `diagram.settings.update`, set the patch value to `null` to clear it. `display: "bracket"` preserves the old ticked, fixed-view appearance, and `labelPosition` remains a legacy coordinate-level repair for that mode. Spherical caps use `kind: "sphereCap"` with `center`, `radius`, `height`/`depth`, and `axis`/`normal`; include a visible segment/dimension label `$h$` when the source names cap depth h, and do not represent a cap as a full sphere unless the source actually shows the whole sphere. Use `show: false` to hide helper graph3d points/segments/solids, not `visible: false`. Do not leave a source prism, pyramid, cone, cylinder, sphere, or spherical cap as a camera-only placeholder. Do not add 2D graph functions, metadata `axisLabels`/`showAxes`/`showGrid`, `xAxis`/`yAxis`/`zAxis` helper points or axis-label segments, or vector2d metadata to a `graph3d` diagram.

For school measurement solids, use a clean textbook-style object like a hand sketch, not a coordinate scene. Set top-level `showAxes: false` (and normally `showAxisLabels: false`), hide construction points, and show only the edges or silhouettes needed to read the solid. An outline cylinder or cone uses its circular end curve(s) plus the two live camera-facing outside generators. Use `renderStyle: "outline"` for a deliberately unfilled curved-solid sketch. Use `renderStyle: "surface"` on both components of a joined cone and spherical cap when the object must read as a true 3D composite: Mauth renders depth-ordered shaded faces and omits the internal shared base when the cut-circle geometry matches exactly. A sphere's orthographic silhouette is correctly circular at every angle: use `renderStyle: "outline"` for the clean circle-and-radius convention, or `renderStyle: "surface"` when a light fill and restrained great-circle guides provide useful spatial cues. The renderer resamples camera-facing curved-solid strokes and refreshes screen-offset labels throughout pointer rotation; neither should wait for the camera-save callback on release. It also guarantees a 1:1 horizontal/vertical board scale and expands `xRange`, `yRange`, and `zRange` about their existing centres to one equal-span cube before projection. Here 1:1 is an equal-scale rule, not a square-frame rule: `widthPx` and `heightPx` may remain independent for page composition, and the spare dimension becomes whitespace rather than stretching the object. Do not fake perspective with unequal scales. Dimension labels and guides are black unless the source deliberately uses another colour. Put `display: "label"` on a visible solid edge when the edge itself communicates the measured length. Put `display: "guide"` on a radius, diameter, central axis, perpendicular height, or another measurement without a suitable visible outside edge. Increase `labelOffsetPx` when the label needs more separation; do not use `labelPosition` for new label/guide annotations. Keep `display: "bracket"` for old or deliberately fixed-view diagrams only. Orient a prism and its camera so the length dimension visibly follows the extrusion instead of resembling a cross-section height. Choose the camera, range centres, and frame size together with enough margin for the annotations, then compare the Student copy during a held drag with the same view after release to confirm that the object and labels do not snap. Top-level `showAxes` and `showAxisLabels` are supported display controls; only their old placement inside `metadata` is invalid.

For an exact joined cone and spherical cap in surface mode, Mauth combines both editable components into one opaque depth-ordered mesh at render time. The internal bases are omitted, the shared cut-circle is drawn once, and only the cap profile exposed below that seam is outlined. This prevents a separately layered sphere silhouette from appearing through the cone.

In manual Solutions mode, newly added graph3d points, segments, and dimensions are solution-only by default. `data.points`, `data.segments`, `data.dimensions`, `data.faces`, and `data.solids` may each store `solutionOnly: true`; aliases such as `vertices`, `edges`, `dimensionLines`, and `surfaces` remain supported. The shared schema, `diagram.settings.update`, editor, validator, and preview preserve this state. Prefer element-level annotations for a few answer points or guides and a paired solution diagram when most of the 3D construction changes.

In teacher-facing 3D controls, perpendicular/right-angle relationships live in one top-level **Annotations** section after **Dimensions**. Each row identifies the owning dimension and offers only connected dimensions or compatible perpendicular faces under **Perpendicular to**. This geometric definition is not duplicated in Settings. The saved relation remains `rightAngleWith` on the owning `data.dimensions` entry; face targets use the namespaced form `face:<face-id>`.
...
Image annotations are structured entries in `data.annotations`. Supported kinds are `label`, `ellipse`, and `arrow`; positions and dimensions are percentages of the configured image box so overlays stay aligned when the image is resized. In manual Solutions mode, new annotations default to `solutionOnly: true`, disappear from Student mode, and render in solution blue without recolouring the bitmap or shared annotations. Use this shared-image layer for focused labels, circles, and arrows. Use a paired solution image only when the bitmap itself must be replaced or the whole image must differ independently.
...
For a blank or partially completed diagram response surface, the manual editor exposes **Complete in solutions** directly on the student diagram panel. It creates one editable solution-only copy in the same replacement slot and switches to it; later clicks reopen that copy instead of adding another. This also works for diagrams nested inside columns. Use the paired copy for whole completed curves or charts, substantially different Venn/network/geometric constructions, custom Penrose Substance, unsupported complex Penrose predicates, independently completed vector or 3D diagrams, and wholly replaced or independently completed images. Use element-level `solutionOnly` annotations when a shared diagram needs only the supported points, labels, segments, shading, image overlays, or other focused answer elements described above.
...
For a structured geometry diagram, points and segment relationships may store `solutionOnly: true`. New solution points and segments created in Solutions mode use that layer automatically. Student mode removes those points and any relationships that depend on them; Solutions mode renders only the answer points, segments, and labels in solution blue. Keep custom Advanced Substance separate from this structured path. Use a paired solution diagram when the answer depends on custom Substance or complex predicates such as angle/equality constraints rather than a few added points or segments.
...
Venn region records may store `solutionOnly: true` when their label/value or shading is an answer. Student mode keeps the same fixed two-set or three-set region slot but blanks the answer and removes its shading, so the diagram does not reflow. Solutions mode shows that region answer in solution blue. Use a paired solution diagram for custom Substance or a wholly different completed set diagram.
...
Use `network` for schematic network diagrams where layout is conceptual rather than coordinate-accurate. Use `VectorSegment(name, start, end)` for directed links and `Segment(name, start, end)` for undirected links. If a link connects a node to itself, use the same start and end node; Mauth renders this as a visible loop.
...
Structured network nodes and links may store `solutionOnly: true`. New nodes and links added in Solutions mode use that layer by default; Student mode removes them and links that depend on hidden nodes, while Solutions mode colours only those answer elements and labels blue. Use a paired solution diagram for a custom Advanced Substance network or a substantially different completed network.
...
Manual solution annotations stay inside that same `metadata.vector2d` structure. Vectors, `segmentLabels`, and `angleMarkers` may each store `solutionOnly: true`; the shared schema, `diagram.settings.update`, editor, and preview all preserve it. Prefer this element-level layer for a few added answer vectors or markings. Use a paired solution-only diagram when most of the construction changes.
...
- Blank answer/completion tables are shared response surfaces with sparse `solutionEntries`; given-data tables stay shared without answer metadata. Legacy student/solution table pairs remain readable but are not the normal authoring path.
- Treat `:::space` blocks as student answer/work space by default. In marked free-response questions, parts, and subparts, the editor's Add -> Answer + solution action is the normal response-space action: it creates a paired answer-space block followed by a solution text block that starts with a blank line and then the worked steps. Paired solution slots default to blank answer space, not ruled lines; turn on `showLines` only when lines are deliberately wanted. The Add -> Solution slot action creates the same pair in places where a separate explicit slot action is shown; adjust the prompted line count when the default is not enough.
...
- In every standard test and exam booklet, use one question per page wherever the question can fit legibly. Add page breaks at question boundaries and enlarge the existing part/subpart answer spaces so each question has generous working room on its page, with extra lines weighted toward higher-mark parts. If a question cannot fit on one page, split only before a part or subpart using `pageBreakBefore`.
