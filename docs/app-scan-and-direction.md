...
| Desktop packaging    | Electron packaging, Hardened Runtime, per-launch API authentication, Developer ID signing, notarization, and fail-closed release tooling exist. | Distribution broadens beyond the current Apple Silicon alpha or automatic updates become necessary.                             |
...
4. Add package-update and rollback policy only when releases become regular.
