...
- Fresh broad layout-check prompts now use a backend native fast path that emits `mauth.layout.check` directly without spending provider tokens. Repair prompts still go to the provider after the native checker returns concrete warnings.
...
- The assistant panel now surfaces local tool-result state explicitly: committed/completed, preflight failed with no commit, committed but needs repair, or committed but needs review. Provider final text is suppressed during tool turns so it cannot hide a failed or repair-required local Mauth action.
...
- Assistant-facing guidance now exposes first-class `geometry2d` for source-faithful no-axis 2D geometry sketches, and preview summaries report geometry point/segment/arc/angle/decoration counts so semantic review can inspect these diagrams without another paid vision pass.
