...
- The backend now offers direct provider tools for focused authoring: `mauth_question_upsert`, `mauth_author_add_diagram`, `mauth_author_ensure_solutions`, and `mauth_author_adjust_response_spaces`. They map to the frontend's high-level adapter actions and should be preferred over low-level action batches for single-question writing, diagram follow-ups, compact solution requests, and focused answer-space edits.
...
- More high-level native tools:
  - Baseline response-space adjustment is implemented through `mauth.author.adjustResponseSpaces`.
  - Baseline formatting/layout adjustment is implemented through `mauth.format.apply`.
  - Add more compact teacher-task tools so the assistant does not hand-build huge action JSON: repair diagram by id, write all solutions, run layout/print checks, convert one source page/question, and apply exam front matter.
