...
- Successful `mauth.author.ensureSolutions` and `mauth.author.adjustResponseSpaces` calls can finish locally in the frontend. Diagram-bearing `mauth.author.replaceQuestion` and `mauth.author.addDiagram` calls now return a compact semantic-review packet instead of immediately ending the chat: the provider compares the teacher request, focused question text, and actual diagram summary, repairs mismatches once with a focused high-level tool, and only then reports success. Failed calls still go through the provider repair loop.
...
- The backend now offers direct provider tools for focused authoring: `mauth_author_replace_question`, `mauth_author_add_diagram`, `mauth_author_ensure_solutions`, and `mauth_author_adjust_response_spaces`. They map to the frontend's high-level adapter actions and should be preferred over low-level action batches for single-question writing, diagram follow-ups, compact solution requests, and focused answer-space edits.
