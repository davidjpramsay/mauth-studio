...
- Add renderer-level diagram inspection and repair after payload validation. A first conservative Penrose circle-geometry semantic pass is implemented in `mauth.preview.inspect`; remaining work includes label collision checks, graph extent checks, visual screenshot/metadata inspection, pedagogical correctness checks for more Penrose families, Plotly/JSXGraph output checks, and print/output fit checks.
