...
- Fresh broad layout-check, document-inspection, validation, help, and clarification prompts now use backend native fast paths without spending provider tokens. These deterministic routes can run even when the OpenAI provider key is not configured. Repair prompts still go to the provider after a native checker returns concrete warnings.
