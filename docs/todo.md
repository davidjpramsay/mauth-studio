...
- Implemented high-level artifact answer surfaces for assistant-authored questions: `answerSurface: "diagram"` and `answerSurface: "table"` pair blank/partial student diagrams or tables with completed solution diagrams or tables in the same replacement slot. Next improvement is to expose manual editor controls for converting an existing table/diagram into a paired answer surface without using the assistant.
