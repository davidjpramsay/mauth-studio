...
- More diagram-specific inspectors:
  - Extend the shared inspection layer beyond current renderer intent, missing image source, scalar-product vector-label checks, rendered failures, and Penrose circle-geometry semantics.
  - Add targeted inspectors for `graph2d`, `vector2d`, `statsChart`, `setDiagram`, and more Penrose cases. These should check axes/ranges, labels, vector components, chart data interpretation, Venn labels/shading/counts, geometry predicates, and common source-conversion mistakes.
