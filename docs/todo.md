...
- Fresh selected/current preview-inspection, broad layout-check, document-inspection, validation, help, and clarification prompts now use backend native fast paths without spending provider tokens. These deterministic routes can run even when the OpenAI provider key is not configured, and successful read-only local tool outputs now end visibly in the panel instead of making a provider summary call. Repair prompts still go to the provider after a native checker returns concrete warnings.
...
- Automatic post-edit repair loop:
  ...
  - Next: extend the same repair loop around `mauth.layout.check` results so broad layout/print-check prompts can automatically repair safe issues once before reporting.
