...
- Extend the implemented `mauth.preview.inspect` selected-context tool with remaining browser state that is not structural yet: file path, solution visibility, print mode, and deeper diagram-specific semantic checks. Browser-rendered page occupancy, selected-anchor bounding boxes, diagram render failures, solution-slot fit, and L-shaped response-space metrics are now exposed when the preview pane is mounted.
