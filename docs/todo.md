...
- More diagram-specific inspectors:
  - Baseline implemented for `graph2d`, `vector2d`, `statsChart`, `setDiagram`, scalar-product Penrose ray diagrams, rendered preview failures, and Penrose circle-geometry semantics.
...
- Add renderer-level diagram inspection and repair after payload validation. A first shared diagram inspection layer is implemented in `mauth.preview.inspect` and assistant commit preflight: it reports expected-renderer mismatches, missing image sources, missing scalar-product vector labels, rendered diagram failures, likely label collisions, clipping/page-fit warnings when preview metrics are mounted, and conservative Penrose circle-geometry semantic warnings. Remaining work includes graph extent checks, richer visual screenshot/metadata inspection, pedagogical correctness checks for more Penrose families, Plotly/JSXGraph output checks, and print/output fit checks.
