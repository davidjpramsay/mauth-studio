...
- `pnpm eval:assistant:live` runs a small paid live-provider eval against the configured OpenAI model. `pnpm eval:assistant:live:all` runs the current focused suite: circle-geometry question authoring, circle/tangent diagram follow-up, tangent-parallel-chord diagram routing, multipart probability authoring, and focused solution creation. Use it sparingly because it calls the real configured model.
