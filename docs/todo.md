...
- Diagram authoring now has the first automatic post-edit repair loop. After `mauth.author.addDiagram` or a question replacement that includes diagrams commits, the adapter runs `mauth.preview.inspect`; repairable diagram warnings are returned as failed tool output with `validationIssues`, including the `diagramId` to replace, so the provider gets one chance to repair before any success message is shown.
- The post-edit diagram loop now waits for the browser preview to repaint when the preview is mounted, then runs `mauth.preview.inspect` with fresh rendered metrics. This lets the assistant catch painted failures such as failed render placeholders, likely label collisions, clipped diagrams, page overflow warnings, solution-space overflow, and missing L-shaped diagram/answer-space outlines before reporting success.
...
- Automatic post-edit repair loop:
  - Baseline implemented for high-level diagram edits: diagram edit -> commit through history/autosave -> wait for browser preview paint when mounted -> preview inspect with fresh rendered metrics -> return repairable warnings once -> only then report success.
  - Next: expand the repair loop beyond diagrams to focused layout/solution-space passes once the assistant has more high-level layout repair tools.
