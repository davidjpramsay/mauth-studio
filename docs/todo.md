...
- Successful `mauth.author.replaceQuestion`, `mauth.author.addDiagram`, `mauth.author.ensureSolutions`, and `mauth.author.adjustResponseSpaces` calls now finish locally in the frontend. The chat panel shows the adapter's result message and does not spend a second provider round merely to summarise that the edit was done. Failed calls still go through the provider repair loop.
