...
- Automatic post-edit repair loop:
  ...
  - Broad layout/print-check prompts now run `mauth.layout.check` with `autoRepair: true`, repairing safe deterministic issues once before reporting remaining issues.
  - Next: expand automatic layout repair for rendered page overflows only when the repair target is unambiguous from preview metrics.
