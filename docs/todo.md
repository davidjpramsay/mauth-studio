...
- Right-click `Use in assistant` now sets a first-class active target card instead of pasting `@mauth[...]` reference plumbing into the request field. The provider still receives the target through structured context, and `Copy for assistant` remains available for manual target references.
