...
- Automatic post-edit repair loop:
  - Baseline implemented for high-level diagram edits: diagram edit -> commit through history/autosave -> preview inspect -> return repairable warnings once -> only then report success.
  - Next: add a wait-for-paint inspection path for browser-rendered metrics so freshly rendered visual failures, label collisions, and print-fit warnings are caught with less reliance on stale preview measurements.
...
- Add renderer-level diagram inspection and repair after payload validation. A first shared diagram inspection layer is implemented in `mauth.preview.inspect` and assistant commit preflight: it reports expected-renderer mismatches, missing image sources, missing scalar-product vector labels, rendered diagram failures when preview metrics are mounted, and conservative Penrose circle-geometry semantic warnings. Remaining work includes label collision checks, graph extent checks, visual screenshot/metadata inspection, pedagogical correctness checks for more Penrose families, Plotly/JSXGraph output checks, and print/output fit checks.
