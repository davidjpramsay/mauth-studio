...
3. creates the application window immediately and presents a restrained bundled loading page;
4. polls the minimal `/api/health` route until the sidecar can accept requests;
5. generates a random per-launch bridge token and writes it with the URL in a private runtime manifest under the operating system's application-data directory for Codex/MCP discovery;
6. navigates the same window to the built editor served beside the API on the local origin; and
7. removes the manifest and stops the sidecar when the app quits.
...
The initial `BrowserWindow` is created hidden only long enough for the local loading page to render, then presented by the first of Electron's `ready-to-show` or main-frame `did-finish-load` signals. This makes the application responsive before Python finishes importing the maths stack without showing a blank or partly rendered editor. The loading page has no external dependencies, honours the system light/dark preference and reduced-motion setting, and is replaced in the same window once health passes. File-open events stay queued until the real editor origin finishes loading, so the transient page cannot consume a document request. Reactivating the app or opening a second instance restores and focuses the existing window rather than leaving a healthy renderer hidden.
...
## Renderer Loading And Print Safety

`PreviewDiagram` loads JSXGraph, 3D, vector, statistical, and Penrose-backed components on demand. Measurement placeholders preserve frame dimensions without loading an engine. Suspense placeholders and renderer failures participate in print readiness; a failed diagram cannot silently become a blank printed area. `functionGraphExpressions.ts` owns expression conversion and implicit-relation snapping shared by Content and rendering, so editing a blank document no longer imports JSXGraph through a helper.

Penrose responses and imported images expose loading/ready/error states beyond component loading. A changed Penrose request cannot reuse a previous request's readiness, aborted responses cannot replace current geometry, and an empty SVG response is an error. Image load failures and missing image sources also block printing.

The Print button and Command/Ctrl-P share a bounded preflight. Diagram readiness, fonts, and two settled layout ticks share one deadline, with timer fallbacks for background windows. Errors or timeout show Retry/Cancel instead of opening Print. Retry remounts the print surface. Document edits, tab changes, and copy changes invalidate pending work; cancellation restores the window title. Browser-native print commands outside this app-controlled path cannot be cancelled by `beforeprint`.

`pnpm test:visual` compares representative Student and Solutions print pages, exports actual PDFs, and checks compact print failure/retry and blank-document lazy loading. Run `pnpm build:web` first. `pnpm benchmark:web` records non-gating first-graph, copy-switch, and editing timings and resource sizes for a 30-question fixture; first-graph timing is not full-document completion. Both use mocked API traffic and disposable browser storage, never the teacher's active folder. Baselines are platform-specific; review every changed image before deliberately updating them with Playwright's `--update-snapshots` option.

`MAUTH_DESKTOP_USER_DATA` optionally selects an absolute diagnostic profile, including isolated recovery, runtime manifest, and instance lock. Pair it with `MAUTH_DOCUMENTS_ROOT` for disposable native QA. Normal launches retain the platform's conventional application-data directory. `pnpm smoke:packaged-opening` exercises the built bundle and bundled connector with an isolated profile; it never changes the normal installed profile. `MAUTH_PACKAGED_APP` can target another local bundle. Native quit confirmation is stubbed only inside this test process; real cloud hydration and signed in-app updates require separate verification.
...
`useEditorDocumentTabsController` composes several document sessions inside that one editor window. A tab captures the structured document, independent undo/redo stacks, selection/navigation anchors, folder/project identity, active path and loaded revision, save fingerprints, conflict/status state, and dirty flag. Switching captures the outgoing tab before restoring the target; an unavailable external folder does not erase the restored tab's cached contents. If the active recovery draft is newer but its project lookup failed, merge the folder identity from its matching session tab without replacing the newer draft. Opening a saved file activates an existing identity-matched tab or creates one, while New Document creates a draft tab. Closing activates the requested tab, runs the same save/discard/cancel guard, removes only that session, and restores the adjacent tab. The desktop File menu sends Command-W through the narrow preload bridge to this same guarded active-tab close path; after every document has closed, Command-W may close the empty window, while Command-Shift-W remains the explicit window-close shortcut. The active tab alone owns preview, print, file sync, and the live React bridge state.
...
While a saved document is open, `useActiveProjectFileSyncController` polls a targeted file-summary endpoint through an explicit active-file outcome contract. Requests carry the owning folder identity, and pending reads must recheck tab id, path, revision, and fingerprint before applying anything. Storage/network failures are absorbed as `unavailable`; a missing file and a newer dirty file remain distinct. Mauth reports reconnection only when the active file is confirmed current or safely reloaded. Save completion updates only its originating tab's file metadata and save baseline, never another tab's document or subsequent edits.

Opening another document preserves the current tab rather than invoking a replacement transition. Finder requests wait for recovery hydration and other file operations, then use an ordered queue and the atomic `open-document` endpoint. That endpoint validates the requested file and project index before selecting its folder. Unavailable cloud indexes produce a named, retryable error dialog; the requested file is never confused with a still-visible recovered draft. A successful open creates or activates a tab, preserving an already-open dirty copy.

Folder switches and active-file version restores still use `projectFileBeforeOpenWorkflow.ts`: save, save a recovery copy, deliberately continue without saving, or cancel. Cancellation and recovery-copy failure block the requested replacement. `projectFileVersionRestoreWorkflow.ts` enforces this guard before invoking the disk restore; inactive-file restores do not disturb the current document. Draft and tab-session backups are independently serialized and automatically retried, without depending on access to cloud project metadata.
...
Project saves use base revisions. Complete revision checks, version snapshots, index reconciliation, and writes share a reentrant process lock and a bounded local file lock across app processes using the same private state root. Revision-aware moves retain file identity, versions, and old-path tombstones. Tab-bound requests carry `documentsPath` so changing the selected folder cannot redirect an in-flight read or write. This is not a cross-computer cloud lock. See `docs/storage.md` for the full storage contract and recovery rules.
...
Normal and arbitrary-density `statsChart` surfaces also use `data.regions` for deterministic areas under the base curve. Region modes cover an interval, either one-sided tail, or both outside tails. The Plotly adapter interpolates the curve at exact authored bounds, puts fill traces behind the shared curve, and suppresses boundary strokes. Regions participate in the same Student/Solutions filtering and solution-blue recolouring as supplemental series. Focused `diagram.settings.update` region operations support update, upsert, and delete without replacing the chart. New normal charts use a slightly wider default domain of approximately `mean ± 3.2 standard deviations`; explicit source ranges remain authoritative. Axis numbers and short outward tick marks are renderer-owned and remain aligned to the same tick positions. Plot layout margins, outside tick placement, and a small expanded SVG clip allowance protect ticks, numbers, and curve strokes during preview and print scaling.
