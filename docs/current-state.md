...
- **Current source state:** signed and notarized `v0.1.3` is the current public Apple Silicon alpha at tag commit `7448cde`. It includes native `.mauth` Finder identity/Quick Look, sharper screen previews, multi-document tabs, explicit agent document targeting, native menu ownership of System Status/appearance/solution validation, compact toolbar controls, System Status contrast repair, and reusable-logo reconciliation. The public website, release notes, DMG, ZIP, ZIP-only updater metadata, and ZIP blockmap were verified after publication. Release preflight validates the selected Apple credential with a live, read-only `notarytool history` request before starting a future full build.
...
Handoff status: standalone implementation, signed/notarized release publication, teacher-confirmed updates, and draft-first publication are implemented. Public 0.1.3 contains the authoring, connector, Finder/Quick Look, sharper-preview, multi-document tab, native-menu, status-contrast, and reusable-logo slices described below. `.mauth` remains versioned JSON, exports `au.edu.acc.mauth-studio.document`, uses a dedicated document icon, and packages sandboxed Swift thumbnail and preview extensions. Finder thumbnails use Core Graphics; Spacebar previews use a fully native AppKit text surface so no WebKit/network subprocess or embedded PDF is required.
...
runtime: one active local Developer ID signed 0.1.2 checkpoint installed in `~/Applications` with native Finder extensions and centred document artwork; public 0.1.3 is available through the updater and signed DMG
...
worktree: clean at this checkpoint; the 0.1.3 Finder/Quick Look, sharp screen-preview, multi-document tab, native desktop-menu, System Status contrast, and reusable-logo reconciliation slices are committed and published together
```
...
- Public 0.1.2 was built from clean, pushed `main`, published at tag commit `aaf63ce`, and verified against the public release assets and updater metadata. Preserve any new uncommitted work found in a later checkout.
...
64. **Completed standalone agent-connector packaging and public 0.1.2 release.**
    ...
    - Focused connector planning and desktop tests pass. The guarded clean-main release completed Developer ID signing, app and DMG notarization/stapling, Gatekeeper checks, connector smokes, public updater-asset verification, and prerelease publication at `v0.1.2`.

65. **Fixed overlapping manual saves after the first Investigation 2 save.**
    - The first project-file write completed successfully, but an immediate duplicate save reused the pre-save revision and returned `409 Conflict`, replacing the successful status with a false `File changed on disk` message.
...
70. **Reconciled legacy reusable Grace logo records.**
    ...
    - The rendered tab smoke seeded the three legacy Grace records and verified the visible list reconciles to **Grace Christian College**, **Cornerstone Christian College**, and **Australian Christian College**. `v0.1.3` was then shipped from clean pushed `main`, accepted by Apple for both app and DMG notarization, stapled, Gatekeeper-verified, published with hash-verified updater assets, and confirmed on the public download site. A post-publication byte check caught and corrected Electron Builder's pre-stapling DMG metadata entry; the durable pipeline now publishes ZIP-only updater metadata and rejects extra artifacts.

The previous full `pnpm check` baseline after the overlapping-save fix was API 82, web/actions 601, Plotly 8, launcher/runtime 43, and a passing TypeScript/Vite production build. Public `0.1.3` raises launcher/runtime coverage to 47 and adds three native Quick Look tests; the final complete gate is recorded under **Current Verification Baseline**. The generated connector and packaged connector previously completed live MCP snapshot smokes, and the complete signed/notarized public macOS release passed packaged-app, stapling, Gatekeeper, public-asset, and updater-metadata verification. The current Investigation 2 document was visually verified as one shared standard-test title/brief page plus two linked rubric pages. Its saved visible file also served as the native Finder fixture: the installed local checkpoint rendered the dedicated thumbnail and a nonblank Spacebar summary with the correct school, subject, title, subtitle, 20 marks, and task text. Public `0.1.3` contains that Finder implementation; the local running app remains on the earlier checkpoint until its update is approved. Recheck external cloud-folder materialisation before teacher-file authoring.
...
1. Clean-machine verify the published `0.1.3` DMG, dedicated icon, Finder thumbnail, Spacebar summary, tab recovery, bundled connector document targeting, and Grace logo reconciliation. The guarded build, Apple notarization/stapling, Gatekeeper checks, remote asset hash verification, updater metadata, GitHub prerelease, and public download page have already passed.
...
5. A local Developer ID signed `0.1.2` checkpoint containing the earlier Finder work is still running; it does not contain the new tabs or logo reconciliation. Public `0.1.3` is available through the updater and signed DMG. Use `pnpm dev:status` before opening another copy, allow the teacher to approve the app update/restart, and do not start manual API/web terminals alongside it.
