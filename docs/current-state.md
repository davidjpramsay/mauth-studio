...
- **Current source state:** `v0.1.1` remains the installed public updater bootstrap. The `v0.1.2` source candidate, including all authoring changes described below and the bundled standalone Mauth Agent Connector, passed CI and was merged through PR #39 at `6f8a62c`. The guarded signed/notarized release remains to be published. Release preflight now validates the selected Apple credential with a live, read-only `notarytool history` request before starting the full build.
...
- The 0.1.2 source candidate and release-alignment documentation are committed and merged through PR #39. Preserve any new uncommitted work found in a later checkout.
