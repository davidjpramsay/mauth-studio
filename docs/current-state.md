...
The subsequent user-requested retry built and successfully notarized the 0.1.6 app, then created the ZIP, DMG, and blockmaps. DMG notarization failed with the same missing-Keychain-profile error, including one direct retry of that submission. This supersedes the earlier unnotarized-app status above, but the complete release remains unpublished. Preserve the generated artifacts while resolving credential access; the remaining release gates are DMG notarization/stapling/validation, ZIP-only updater metadata normalization, distribution verification, and draft-first remote asset verification/publication. Do not distribute the unfinished artifact set or treat app approval as DMG approval.
