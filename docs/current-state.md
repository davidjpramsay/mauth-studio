...
- **Current source state:** signed and notarized `v0.1.2` is the current public Apple Silicon alpha release. Its five updater and installer assets were published and independently re-read from GitHub at tag commit `aaf63ce`; the public website points to the same release. This Mac still has the `v0.1.1` updater bootstrap installed until the first in-app update is accepted. Release preflight validates the selected Apple credential with a live, read-only `notarytool history` request before starting the full build.
