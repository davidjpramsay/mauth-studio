import { FolderOpen, PlusCircle, Save } from "lucide-react";

import type { HeaderSaveStatus } from "@/hooks/useProjectFileStatus";
import { Button } from "@/components/ui/button";
import { DocumentTabRail } from "@/components/header/DocumentTabRail";
  ...
  return (
    <div className="flex min-w-0 flex-1 items-center gap-2 rounded-md border border-blue-300/20 bg-white/[0.05] p-1">
      {tabs.length ? (
        ...
        <>
          <span
            className={cn("ml-1 size-2 shrink-0 rounded-full", storageStatusTone(saveStatus))}
