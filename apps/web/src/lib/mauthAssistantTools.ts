...
function parsePenroseSubstance(source: string): PenroseSubstanceSummary {
  ...
  for (const rawLine of source.split(/\r?\n/)) {
    ...
    const declarationMatch = line.match(/^([A-Z][A-Za-z]*)\s+(.+)$/);
    if (declarationMatch) {
      const type = declarationMatch[1];
      const names = declarationMatch[2]
        .split(",")
        .map((name) => name.trim())
        .filter(Boolean);
      const bucket = declarations.get(type) ?? new Set<string>();
      for (const name of names) bucket.add(name);
      declarations.set(type, bucket);
    }
  }

  return { source, declarations, labels, hiddenPoints, calls };
}
...
function penroseCircleGeometryWarnings(
  entry: PreviewBlockEntry,
  questionText: string,
  summary: PenroseSubstanceSummary,
): { checks: string[]; warnings: MauthPreviewInspectionWarning[] } {
  const checks: string[] = [];
  const warnings: MauthPreviewInspectionWarning[] = [];
  const intent = diagramIntentFromText(questionText);
  const isCircleGeometryIntent = intent?.id === "schematic-geometry" || /\bcircle\b/i.test(questionText);
  if (!isCircleGeometryIntent) return { checks, warnings };

  const addWarning = (code: string, message: string, severity: MauthPreviewInspectionWarning["severity"] = "warning") => {
    warnings.push({ code, severity, anchor: entry.anchor, targetId: entry.block.id, message });
  };
...
function diagramSemanticInspection(entry: PreviewBlockEntry, questionText: string) {
  ...
  if (entry.block.graphConfig.type !== "geometricConstruction")
    return { checks: [] as string[], warnings: [] as MauthPreviewInspectionWarning[] };

  const source = penroseSourceFromGraphConfig(entry.block.graphConfig);
  if (!source.trim()) {
    return { checks: ["penrose-source"], warnings: [] };
  }
  const summary = parsePenroseSubstance(source);
  return penroseCircleGeometryWarnings(entry, questionText, summary);
}
