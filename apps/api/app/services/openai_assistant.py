    ...
    if asks_to_write_question:
        ...
        if has_source_attachment and any(term in text for term in ("slope field", "direction field", "dy/dx")):
            diagram_guidance += (
                ...
                "and highlightedPoints for any source point where the student must calculate or draw a slope segment; "
                "ordinary point features alone are not enough. Put graph bounds, display flags, functions, "
        ...
        "properties": {
            ...
            "graphConfig": {
                ...
                "description": (
                    ...
                    "BT/\\overleftrightarrow{BT} as \\overrightarrow{BT}, and do not paste PDF-extraction control "
...
def source_conversion_native_diagram_rules(diagram_fields_enabled: bool, diagram_types: list[str] | None) -> str:
    ...
    if "geometricConstruction" in allowed:
        lines.append(
            "- For geometricConstruction/Penrose source diagrams, preserve named points, lengths, angles, tangents, chords, and relationships with supported Substance instead of a prose diagram description. Label declared points directly, e.g. Label L $L$, not Label LLabel $L$. Use RightAngle(A, B, C) for visible right-angle markers; PerpendicularToSegment needs a declared Line first, not a NamedSegment. Do not emit options.styleSource/domainSource; Mauth supplies the Penrose preset style."
        )
    if "setDiagram" in allowed:
        lines.append(
...
- Use native table/table fields for source and answer tables; do not put Markdown pipe tables in text or solutionText.
