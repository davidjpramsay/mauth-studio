...
- Source scalar-product/vector-ray diagrams with magnitudes, angle markers, and no coordinate axes should use diagram.graphConfig.type = "geometricConstruction" with supported Penrose Substance. Do not use vectorRelationship for these; vectorRelationship is for conceptual network/link diagrams only. Preserve visible right-angle markers with the same two rays shown in the source, preserve numeric angle labels such as $45^\\circ$, and preserve opposite collinear vectors when the source shows them. Draw right-angle markers with RightAngle(pointOnFirstRay, vertex, pointOnSecondRay); do not use PerpendicularToSegment for that marker. Do not invent unsupported predicates such as SegmentLength, LabelsPoint, OppositeRays, Collinear, or Connect; show given magnitudes with Label plus LabelsSegment. If the source shows two vectors as opposite rays on the same straight line, declare the line first, e.g. `Line lineAD`, then use `LineThrough(lineAD, A, D)` and `On(O, lineAD)`. For a circle through named points, use a hidden centre plus `CircleThrough(omega, centre, A)` and `OnCircle(B, omega)`, `OnCircle(C, omega)`; do not call `CircleThrough(omega, A, B, C)`. In replaceQuestion/addDiagram diagrams, always wrap renderer payloads inside graphConfig; never put type/data/options directly on diagram and never use config as an alias.
